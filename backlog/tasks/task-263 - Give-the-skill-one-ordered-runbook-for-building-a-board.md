...
Reopened for one row, then closed again (commit 9d6637c2).

A skill/rubric divergence, not a wording preference: the frozen baseline's external row (docs/design/skill-evals/baseline/archboard/SKILL.md:161) read 'a caller, library, service, shell or hypothetical part outside the checkout', and the rewritten row dropped 'or hypothetical part' while evals/rubric.md:101 still grades against it and S08 (evals/evals.json:724) asks for a hypothetical Metrics extension. The skill had stopped telling an author where a part that does not exist goes, in the one scenario that turns on it.

Restored in the current row's voice rather than by reverting: 'a caller, library, framework, runtime, shell or hosted service this codebase does not own, or a part nobody has built yet' authors 'a node of kind external, unbound; a part that does not exist yet is drawn and unbound the same way, taking the configured kind the request names for it when it names one.' The qualifier is load-bearing and checked against S08's own outcomes: its node-kind check demands kind , the kind the same request has the author configure, so a row that said flatly 'a part nobody has built is external' would have walked an author into failing it. Runbook step 5 follows the row: the parts to decide include the ones that belong to another codebase or nobody has built yet.

Checked for other contradictions and found none needing an edit: evidence rule 2 and authoring.md already say a planned part stays unbound, the binding row already authors nothing for a part you could not inspect, and propose-compare's worked example already draws a table nobody has built. One line I considered and deliberately left alone: 'Keep it true' says every node has a responsibility the source supports, which reads oddly for a part the request rather than the source justifies — weakening a guardrail line is the wrong price for that, and the catalogue row now says where such a part goes. No planning branch was added to the runbook: that needs a representation decision first (the node schema is strict and has no way to mark a part as proposed; an absent binding cannot mean planned), and the user has approved a separate task for it.

TASK-268 exists to stop this class of drift between the skill and the rubric. evals/ was not edited.
...
SKILL.md now opens with the runbook: 19 numbered steps, one sentence each, that walk an author from an empty request to a board somebody can read, for either diagram type, and route to the recipes, the evidence rules and the catalogue rather than restating them. The material the 2026-09-18 batch showed authors passing over was never unreachable — every candidate run read the recipe that teaches repeat and message kinds — so the fix is sequencing plus two passes the skill never had: step 11 models the subject a second way and keeps the shape whose advantage it can state, and step 18 reads the board back and simplifies it before step 19 reports. Step 9 carries what the one S07 run that passed both checks actually did: the work a part does to decide what to do next is its own step with that part at both ends, separate from the calls it makes per candidate, carrying the count the source fixes. Supporting changes: a flow participants are a subset of the board nodes (defined in sequences-views-walkthroughs.md, instructed at step 7), so an honest fine-grained board and a self step were never in competition; create-sequence.md carries one participant rule instead of two contradictory ones; all four variant-flag files agree that the command line wins and the write warns naming both; and the external catalogue row says again, in its current voice, that a part nobody has built yet is drawn and unbound too, taking the configured kind the request names for it — the clause the rewrite had dropped while evals/rubric.md and S08 still graded against it.

Verified structurally against the shipped file: 19 contiguous steps with one sentence-terminating period each, 16 carrying links, and all 13 link targets resolving. oxfmt --check clean on SKILL.md, create-sequence.md, propose-compare.md and sequences-views-walkthroughs.md; bun scripts/sync-skills.ts syncs both skills byte-identically into .agents/ and .claude/; bun test tests/system/cli/install-targets.test.ts 9 pass. No test file was touched, and evals/ was not edited.

AC3 is checked for its guidance half only. The measured half — S07 passing flow.repeat and flow.message-kinds in a later batch — is pending the user next evaluation batch, which is theirs to run by hand and was never startable from a session. What would count as evidence for the runbook when it comes: S07 runs that READ their guidance carrying both checks, since the one run that passed both in the 2026-09-18 batch is also the only run the report lists as not having read create-sequence.md, so improvement concentrated in guidance-reading runs is the signal and improvement in runs that skipped it is not. TASK-267 owns the other half: whether a self message is truthful for that exchange at all, and making the scenario, the rubric and the harness flow-with-steps check agree. TASK-268 owns stopping the skill/rubric drift the external row showed.
