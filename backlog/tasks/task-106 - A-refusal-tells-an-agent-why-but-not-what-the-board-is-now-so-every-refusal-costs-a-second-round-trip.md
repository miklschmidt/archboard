...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 `BOARD_HELD`, `BOARD_VERSION_CONFLICT` and `CLAIM_REVOKED` refusal bodies carry the board's current elements and version, the same the agent would get by reading it
- [x] #2 The CLI and MCP surfaces print the refusal with the reason first, and expose the board from the same response without a second call
- [x] #3 `test:lock` and `test:version` assert the document is present and current in each refusal
- [x] #4 The excalidraw-skill tells an agent the refusal already holds the board, so it acts on that rather than re-reading
<!-- AC:END -->
...
Final verification after commit c77ed8b: bun run test exited 0. It reran 26/26 push suites including type-check, lock 119, version 65, parity 41 MCP tools against 50 CLI entries, doing 42, MCP stdio 6, module-scope 50, one-write 69, fixed-point zero browser drift, typed-text, and live-session with 42/42 mixed-write document agreements. The three refusal tests compare attached documents and versions to live board reads; the CLI assertion proves reason-first output; parity proves both CLI and MCP use the same refusal formatter. No follow-up remains.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Added document and version to BOARD_HELD, BOARD_VERSION_CONFLICT, and CLAIM_REVOKED without changing their reason fields or taking another lock. CLI and MCP preserve and print that response reason-first; the skill now tells agents to use it instead of re-reading. Verified by lock 119, version 65, parity, doing, MCP wire checks, and the full 26-suite bun run test including browser fixed-point, typed-text, and 42 mixed-write live-session cycles.
