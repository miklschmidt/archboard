...
Own the React-facing presentation adapter in `src/ui/voice-session`. It consumes only the private package state, the sole codex-realtime binding phase, and closed coordinator/session capabilities and exposes render-ready values without media/protocol objects.
...
- [ ] #3 The adapter never reduces protocol events, deduplicates transcripts, owns media, or chooses recovery; it projects the authoritative package/binding state only.
