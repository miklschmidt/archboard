...
7. Remediation 3: retain unresolved mutation ownership by exact child, epoch, and logical-call identity; defer terminalization faults until a canonical prospective response exists; quarantine the epoch before releasing control; extend DynamicToolLifecyclePort with one atomic poison-and-recovery owner that may trigger bounded idempotent terminalization retries or confirm exact child exit; block reconstructed and different calls before ID issuance while quarantined; expose compact quarantine inspection; clear owners only after proven terminality, exact child exit, or explicit dispose; document and enforce the contract with reconstructed replay, recovery, shutdown, exit, dispose, and no-new-effect regressions; then rerun the requested capped validation without changing task status or acceptance criteria.

8. Remediation 4: split unresolved logical-effect ownership from exact JsonRpcRequestId wire ownership; retain one original transport handle per admitted wire up to CODEX_APP_SERVER_CAPACITY.outbound.pendingReverseRequests; deduplicate repeated request IDs; fan the canonical recovered outcome to every same-logical wire exactly once; retain different mutation/read wires with a canonical refusal until recovery; reserve overflow for a shared exact shutdown path rather than admitting an unbounded wire; add synchronous poison-acquisition fallback through exact child/session transport teardown and explicit fatal lifecycle reporting when fallback proof cannot be acquired; clear with zero writes only on exact child exit, exact teardown, or dispose; update compact inspection, authored policy, fakes, and hostile tests, then rerun capped validation without changing task status or acceptance criteria.
<!-- SECTION:PLAN:END -->
...
Remediation 3 completed: unresolved operation terminality now retains one compact owner keyed by exact child, epoch, and logical-call identity. The dispatcher keeps the canonical prospective response pending, poisons the epoch through DynamicToolLifecyclePort, joins same-object and reconstructed replays, blocks every other call in that epoch before ID issuance, and only responds after lifecycle-triggered bounded terminalization proves every owned ID terminal. Exact child exit or dispose rejects and clears the retained owner without a dynamic response; malformed or mismatched exit proof leaves the owner quarantined. Added compact count/state inspection, partial-ID-issuance recovery, post-recovery replay retention, confirmed-effect, repeated-recovery, wrong-child-exit, and no-new-ID/approval/effect regressions. Updated the authored contract and repository policy. Final validation under sequential 6 GiB memory/1 GiB swap transient units: format write passed on 802 files; both TypeScript graphs passed; scoped oxlint passed on 29 files with 0 warnings/errors; fmt:check passed on 802 files; dynamic suite passed 56 tests across 8 files with 335 expectations; authored contract policy passed 11 tests with 332 expectations; repository inventory passed 39 tests with 69 expectations. The known full test:repository OOM service was not rerun and browser owners remain outside this remediation. Task stays In Progress with all acceptance criteria unchecked for independent review.

Remediation 4 implemented on fixed base 8521dd6d9e9e5e21532651b9127d55167b0a2ebd: separated one logical unresolved-effect owner from bounded exact JSON-RPC wire owners retaining the original transport request; same request IDs deduplicate, same-logical fresh wires join before or during quarantine and receive the canonical outcome once, and different mutation/read wires are retained for one canonical refusal without IDs, approval, or effect. The accepted pending-wire cap is CODEX_APP_SERVER_CAPACITY.outbound.pendingReverseRequests (128); overflow starts one exact fail-closed shutdown path without admitting another wire. Poison throws/malformed owners and response-write failures synchronously request exact epoch/session/transport teardown; invalid, rejected, or unavailable teardown ownership leaves quarantine retained and reports a fatal lifecycle fault. Zero-write clearing remains limited to exact child exit, exact teardown, or dispose. Authored policy, compact inspection, lifecycle fakes, simultaneous-arrival/replay/refusal/capacity/poison/teardown/write-failure tests were updated. Validation: bun run type-check passed both projects (archboard-task1430504-rem4-typecheck-final); scoped Oxlint passed (archboard-task1430504-rem4-oxlint-final); bun run fmt:check passed (archboard-task1430504-rem4-fmtcheck-final); dynamic-tools lane passed 64 tests / 378 assertions (archboard-task1430504-rem4-dynamic-final); authored policy passed 11 tests / 337 assertions (archboard-task1430504-rem4-policy-01); inventory passed 39 tests / 69 assertions (archboard-task1430504-rem4-inventory-01). Task intentionally remains In Progress with acceptance criteria unchecked for parent review.
<!-- SECTION:NOTES:END -->
