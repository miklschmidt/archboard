...
<!-- SECTION:PLAN:BEGIN -->
1. Replace the two additionalContext placeholders with the reviewed thread-link reason precedence and operation kind/rpc contract, including exact nullability and lifecycle rules.
2. Extend the repository-policy owner with fixed enum/order/domain checks and independent mutations for reason precedence, nullability, operation tuple closure, rpc ordering, and lifecycle wording.
3. Update fixed digests, record source anchors and focused evidence in task notes, then commit the contract remediation without finalizing the task.
4. Run the focused owner, inventory and boundary policies, complete repository lane, both TypeScript projects, lint, format, and diff/status audits.
<!-- SECTION:PLAN:END -->
