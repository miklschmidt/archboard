...
<!-- SECTION:PLAN:BEGIN -->
1. Replace the partial namespace resets with one @theme-wide reset, then redeclare only exact TASK-140 semantic aliases. Preserve the documented boundary: transition and duration-150 are the two hardcoded transition-default fallbacks, not Archboard tokens or adopted product classes. Unrelated compiler-built utilities remain outside the semantic-theme contract; this leaf neither adopts them as semantic tokens nor prohibits them globally.
...
4. Compile positive candidates for every owned family and refusal candidates for every configurable stock family. Add table-driven negative mutations for imports, Preflight, token-family values, aliases, forced-color opt-out, and reduced-motion loss, plus explicit evidence that the two hardcoded transition-default fallbacks are absent from theme product source.
