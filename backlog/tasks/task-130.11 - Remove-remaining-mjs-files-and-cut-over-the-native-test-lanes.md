...
11. Approved narrow review remediation: add .github/workflows/ci.yml to the edited-file scope for comment-only corrections. Replace the removed test:suites/skip-list wording with test:repository inventory ownership and replace the removed test:browser comment with test:serial-browser. Make no workflow command or behavior change.
<!-- SECTION:PLAN:END -->
