...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Oxlint resolves @/ to ./src/ consistently with Vite and both TypeScript owners; existing area/module-entrypoint/deep-import rules judge the canonical path.
- [x] #2 The smallest native resolver/plugin extension adds no Tailwind rule, warning allowance, second alias table, changing-default mirror, components.json read, or shadcn dry-run.
- [x] #3 Repository fixtures prove valid UI entrypoints pass while deep, cross-area, unknown, and escaping paths fail with existing actionable rule names.
- [x] #4 bun run lint stays deny-warnings clean. TASK-144.04 alone validates components.json agreement and runs shadcn after this alias owner completes.
<!-- AC:END -->
...
Independent review returned REVIEW_CLEAN for the complete immutable range, including a deny-warnings lint replay. Root capped validation passed in archboard-task14418-focused-ee310b2.service (58 tests, 192 expectations, full lint, both TypeScript graphs, formatting, 1.5 GB peak, 0 swap under 6 GB/1 GB caps) and archboard-task14418-repository-ee310b2.service (244 tests, 2,006 expectations, 2.8 GB peak, 0 swap under 12 GB/2 GB caps); neither cap was hit.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Taught the existing Oxlint resolver the single canonical @/ source alias so established area, module-entrypoint, and deep-import rules judge aliased paths. Added hostile fixtures across import forms without adding a second authority, Tailwind rule, warning allowance, or shadcn scope. Independent review and capped focused plus repository validation passed.
