...
**How "required" is enforced without breaking every caller at once.** Stage 1 (TASK-068, TASK-083) collapsed every fan-out into one batched write per intent, so there is now a single boundary where the lock is taken and this can be demanded — which is why this is buildable now and would not have been a week ago. A human's writes arrive through the change report and have no description, and should not be made to invent one.
