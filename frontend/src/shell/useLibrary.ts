...
export function useLibrary(): LibraryController {
  ...
  const offer = useCallback(async (candidate: string): Promise<void> => {
    ...
    try {
      ...
      setPending({ host: fetched.url.hostname, name: libraryName(fetched.url), items: fetched.items })
    } catch (failure) {
  ...
  const reportFromPane = useCallback((next: LibraryItems): void => {
    if (!loadedRef.current) return
  ...
  return {
    items, pending, busy, error,
    acceptInstall, declineInstall, dismissError, reportFromPane, applyFromServer
  }
}
