...
export function BoardNavigator({
  listing, error, currentKey, busy, onSelect, onRefresh, onNew, needsName, onName
    ...
    <aside className="board-nav" aria-label="Boards and variants">
      <div className="board-nav-header">
        <div className="board-nav-title">
          ...
          {listing && <small>{groups.length} boards / {listing.boards.length} variants</small>}
        </div>
        <div className="board-nav-tools">
          <button className="icon-btn" type="button" onClick={onRefresh} title="Refresh boards" aria-label="Refresh boards">
            <Icon name="refresh" size={16} />
          </button>
          <button className="icon-btn" type="button" onClick={onNew} title="New board" aria-label="New board">
      ...
      <div className="board-nav-list">
        ...
        {groups.map((group) => (
          <section className={`board-group${group.variants.some((entry) => entry.key === currentKey) ? ' active-group' : ''}`} key={group.board} aria-label={group.board}>
            <div className="board-group-name">
              ...
              <span className="board-group-copy">
                ...
                <small>{group.variants[0]?.identity.level ?? 'board'} / {group.variants.length} variant{group.variants.length === 1 ? '' : 's'}</small>
              </span>
            </div>
            <div className="board-variants">
              {group.variants.map((entry) => {
                const selected = entry.key === currentKey
                const label = entry.identity.variant === 'current' ? 'Current' : entry.identity.variant
                return (
                  <button
                    type="button"
      ...
      {(scratch || needsName) && (
        <section className="board-group scratch-section" aria-label="scratch">
          ...
          <div className="scratch-card">
            ...
            {needsName && <button className="name-button" type="button" onClick={onName} disabled={busy}>Name this board</button>}
          </div>
        </section>
      )}
    </aside>
