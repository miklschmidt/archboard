...
if (browserAvailable.error) {
  console.error('human-performance: agent-browser is not on PATH, so trusted input cannot be measured.');
  process.exit(2);
}
...
const sourcedAt = Math.max(newestUnder(path.join(repoRoot, 'frontend')), newestUnder(path.join(repoRoot, 'src')));
...
const sessionId = (() => {
  const answer = spawnSync('agent-browser',
...
const tracedServer = spawn('strace', [
  '-f', '-e', 'trace=fsync', '-o', traceFile,
...
let serverStderr = '';
tracedServer.stderr.on('data', chunk => { serverStderr += chunk.toString(); });
...
const fsyncCount = () => fs.existsSync(traceFile)
  ? fs.readFileSync(traceFile, 'utf-8').split('\n').filter(line => /fsync\(/.test(line)).length
  : 0;
const frameElement = async id => {
  const response = await api('POST', '/api/viewport', {
    scrollToElementIds: [id], viewportZoomFactor: 0.5
  });
  if (response.status !== 200) throw new Error(`cannot frame ${id}: ${response.status} ${response.body?.error ?? ''}`);
  await sleep(650);
  return response;
};
const pointOf = (id, edge = false) => evalInPage(`(() => {
  const app = ${APP};
...
const dragFrom = async (point, dx, dy) => {
  ...
  for (let step = 1; step <= 4; step++) {
    await browser(['mouse', 'move', String(Math.round(point.x + dx * step / 4)),
...
try {
  ...
  const upserts = [
    { id: 'drag', type: 'rectangle', x: 0, y: 0, width: 180, height: 90,
      backgroundColor: '#ffffff', fillStyle: 'solid' },
    { id: 'resize', type: 'rectangle', x: 260, y: 0, width: 180, height: 90,
      backgroundColor: '#ffffff', fillStyle: 'solid' },
    { id: 'typing', type: 'rectangle', x: 520, y: 0, width: 220, height: 100,
      backgroundColor: '#ffffff', fillStyle: 'solid' }
  ];
  for (let index = 3; index < FIXTURE_SIZE; index++) {
    upserts.push({ id: `f${index.toString(36)}`, type: 'rectangle',
      x: (index % 100) * 130, y: 300 + Math.floor(index / 100) * 90,
      width: 90, height: 50, backgroundColor: '#ffffff', fillStyle: 'solid' });
  }
  const seeded = await api('POST', '/api/elements/changes?board=performance', {
    clientId: 'fixture', upserts, deletes: []
  });
  check('a large board is seeded entirely through the human change-report route',
  ...
  check('a real headless browser owns the measured pane',
    panes?.paneCount === 1 && /headless/i.test(ua), `${panes?.paneCount ?? 0} pane / ${ua}`);
  const opened = await api('POST', '/api/boards/open', { board: 'performance', reload: true });
  check('the browser renders the complete fixed fixture', opened.body?.elementCount === FIXTURE_SIZE,
    `${opened.body?.elementCount ?? 0} elements`);
  const fullDocumentBytes = JSON.stringify((await api('GET', '/api/elements?board=performance')).body?.elements ?? []).length;
  ...
  check('trusted resizing remains local while a human report is in flight',
  ...
  check('trusted typing remains local while a human report is in flight',
  ...
  const gaps = finalProbe.frames.map(frame => frame.gap).filter(gap => gap > 0).sort((a, b) => a - b);
  const median = gaps[Math.floor(gaps.length / 2)] || 1;
  const reportGaps = finalProbe.responses.flatMap(response => finalProbe.frames
    .filter(frame => Math.abs(frame.at - response.returnedAt) <= median * 8).map(frame => frame.gap));
  const worstReportGap = Math.max(0, ...reportGaps);
  check('report completion has no disproportionate main-thread frame gap',
  ...
  try { process.kill(-tracedServer.pid, 'SIGTERM'); } catch { }
  await sleep(250);
