...
const { measureText, measureLineWidth, canMeasure, faceFileFor } =
  await import(src('core/measure-text.ts'));
const { fontRegistry, familyOf, faceStack, lineHeightOf, loadFace } =
  await import(src('core/fonts.ts'));
...
assert(Math.abs(virgil - 90.54) < 0.01,
...
const KERNED = [
  ...
  ['P.', 2.00],
  ['LT', 1.40],
  ['postgres://primary', 4.00],
  ['Kafka topic: orders.v2', 1.00]
];
...
assert(measureLineWidth('ffi', 20, EXCALIFONT) < measureLineWidth('fif', 20, EXCALIFONT),
...
assert(close(measureLineWidth(' A', 100, LIBERATION), advanceSum(' A', 100, LIBERATION)),
  'a space kerned against the letter after it, which Chrome does not do');

// ── Ignorables ──────────────────────────────────────────────────────────────

console.log('# and a soft hyphen is laid out as nothing');
assert(close(measureLineWidth('a­b', 20, EXCALIFONT), measureLineWidth('ab', 20, EXCALIFONT)),
...
for (const [name, count] of Object.entries({
  Excalifont: 7, Nunito: 5, 'Comic Shanns': 4, 'Lilita One': 2,
  Cascadia: 1, Virgil: 1, 'Liberation Sans': 1, Xiaolai: 209
})) {
  assert(registry.get(name)?.faces.length === count,
...
for (const [number, name] of Object.entries({
  1: 'Virgil', 3: 'Cascadia', 5: 'Excalifont', 6: 'Nunito',
  7: 'Lilita One', 8: 'Comic Shanns', 9: 'Liberation Sans'
})) {
  assert(familyOf(Number(number))?.name === name,
...
// returning a number.
assert(!canMeasure(HELVETICA),
  'Helvetica reports as measurable, and it ships no file for anyone to measure');
assert(canMeasure(EXCALIFONT) && canMeasure(VIRGIL),
  'a family that ships files reports as unmeasurable');
assert(measureText('anything', 20, HELVETICA).missing.length === 8,
...
const declaresA = nunito.faces.filter((face) =>
  face.ranges === null || face.ranges.some(([a, b]) => 0x41 >= a && 0x41 <= b));
assert(carriesA.length > 1 && declaresA.length >= 1 && carriesA[0] !== declaresA[declaresA.length - 1],
...
assert(faceFileFor(0x41, NUNITO) === declaresA[declaresA.length - 1].file,
...
assert(faceFileFor(0x41, EXCALIFONT) === registry.get('Excalifont').faces[0].file,
...
assert(stack.length === 2 && stack[0].length === 7 && stack[1].length === 209,
  `Excalifont's stack reads as ${stack.map((faces) => faces.length).join(' then ')}, not 7 then 209`);
assert(close(measureLineWidth('A', 20, EXCALIFONT),
  measureLineWidth('A', 20, EXCALIFONT)), 'a repeat measurement disagreed with itself');
assert(measureText('A', 20, EXCALIFONT).missing.length === 0,
