...
const cli = (args, stdin) => new Promise(resolve => {
  ...
  child.on('exit', code => {
    let json = null;
    try { json = JSON.parse(stdout); } catch { /* --text output, or a failure */ }
  ...
  socket.on('message', data => {
    ...
    if (message.type === 'pane_open') void shellOpen();
      ...
      void api('POST', '/api/export/image/result', {
        requestId: message.requestId, format: message.format ?? 'png', data: 'aGk='
      });
...
async function drawRow(boardKey, boxes, variant) {
  const payload = boxes.map(([label], index) => ({
    type: 'rectangle', x: index * 300, y: 100, width: 200, height: 100, label: { text: label }
  }));
  ...
  const arrows = ids.slice(1).map((id, index) => ({
    ...
    start: { id: ids[index] }, end: { id }
  }));
  await cli(['add', '--board', boardKey], JSON.stringify(arrows));
  for (const [index, [label, kind]] of boxes.entries()) {
    await cli([
      'promote', '--board', boardKey, '--ids', ids[index],
      '--kind', kind, '--name', label, '--variant', variant
    ]);
  }
  return { added, ids };
}
...
try {
  ...
  await sleep(120);
  check('`board new` puts the source board in the only pane on screen',
    made.code === 0 && made.json?.pane?.place === 'the only pane', made.stderr.trim());
  ...
  check('the palette can be read before anything is drawn',
    palette.code === 0 && /stencil/i.test(palette.stdout), palette.stderr.trim());

  await drawRow('payments', [
    ['API Gateway', 'gateway'],
  ...
  await sleep(150);
  check('branching writes the proposal as its own note',
    branched.code === 0 && branched.json?.board === 'payments@option-a', branched.stderr.trim());
  check('  and reports itself as a branch that moved no pane',
    branched.json?.saveKind === 'branch' && branched.json?.panes?.moved?.length === 0,
    JSON.stringify(branched.json?.panes));
  check('  so the pane holding the source is still holding it',
    source.board() === 'payments', source.board());
  check('  and was sent nothing at all',
  ...
  await sleep(200);
  check('`pane open --board <branch>` puts the proposal on screen',
    beside.code === 0 && beside.json?.board?.board === 'payments@option-a', beside.stderr.trim());
  check('  in a pane that did not exist a moment ago',
    beside.json?.paneCount === 2 && beside.json?.pane?.place === 'right',
    JSON.stringify(beside.json?.pane));
  check('  and the CLI says the other pane was left alone',
    /other pane was not touched/i.test(beside.stderr), beside.stderr.trim());
  check('  which it was: no board reached it',
  ...
  await sleep(80);
  check('  and it is the one holding the branch', branchPane?.board() === 'payments@option-a',
    branchPane?.board());

  const sideBySide = await api('GET', '/api/panes');
  check('the two boards are side by side, one each',
  ...
  check('a third pane is refused, so two is the whole screen', third.code !== 0, `exit ${third.code}`);
  check('  and both panes are still where they were',
  ...
  const cacheAdd = await cli(['add', '--board', 'payments@option-a'], JSON.stringify([
    { type: 'rectangle', x: 300, y: 320, width: 200, height: 100, label: { text: 'Orders Cache' } }
  ]));
  ...
  await cli([
    'promote', '--board', 'payments@option-a', '--ids', cacheId,
    '--kind', 'datastore', '--name', 'Orders Cache', '--variant', 'option-a'
  ]);
  await api('POST', '/api/elements?board=payments@option-a', {
    type: 'arrow', x: 0, y: 0, width: 100, height: 0,
    start: { id: serviceOnBranch.id }, end: { id: cacheId }
  });
  const savedBranch = await cli(['board', 'save', '--board', 'payments@option-a']);
  check('the proposal saves to its own note',
    savedBranch.code === 0 && savedBranch.json?.file !== savedSource.json?.file, savedBranch.stderr.trim());
  const sourceStillHolds = (await api('GET', '/api/elements?board=payments')).body?.count;
  check('  and drawing on it wrote nothing to the source board',
    sourceStillHolds === sourceHeld, `payments held ${sourceHeld}, now holds ${sourceStillHolds}`);
  check('  and moved nothing on screen',
  ...
  check('`screenshot --pane right` photographs the proposal',
    picture.code === 0 && fs.existsSync(shot), picture.stderr.trim());
  check('  by asking the pane holding it',
    branchPane.seen.slice(branchBeforePicture).some(m => m.type === 'export_image_request'));
  check('  and not the pane holding the source',
  ...
  check('`compare` reads the two boards as one architecture and one change',
    diff.code === 0 && diff.json?.summary?.comparable === true &&
    diff.json?.summary?.sharedNodes === 3 &&
    diff.json?.summary?.nodesAdded === 1 && diff.json?.summary?.nodesRemoved === 0,
    JSON.stringify(diff.json?.summary));
  check('  naming the cache as the thing that was added',
  ...
  check('THE SOURCE BOARD IS STILL ON SCREEN at the end of the whole trace',
    Boolean(stillThere), JSON.stringify((await api('GET', '/api/panes')).body?.panes?.map(p => p.board)));
  check('  in the pane it started in, not one it was shuffled into',
    stillThere?.paneId === 'p-source' && stillThere?.place === 'left',
    `${stillThere?.paneId} at ${stillThere?.place}`);
  check('  never having been switched, once, across any of the work on the proposal',
    source.seen.slice(settled).every(m => m.type !== 'board_switched'),
    JSON.stringify(source.seen.slice(settled).map(m => m.type)));
  check('  and it is the board it always was, not the branch under its name',
    (await api('GET', '/api/elements?board=payments')).body?.elements
      ?.every(el => (el.customData?.archboard?.variant ?? 'current') === 'current'));
  ...
  await sleep(250);
  check('back to one pane, holding the source',
    (await api('GET', '/api/panes')).body?.paneCount === 1 && source.board() === 'payments',
    source.board());

  const overwritten = await cli(['board', 'open', 'payments@option-a']);
  await sleep(200);
  check('with one pane, `board open <branch>` is accepted without naming a pane',
    overwritten.code === 0 && overwritten.json?.pane?.place === 'the only pane', overwritten.stderr.trim());
  check('  and it re-points the pane the human was reading',
    source.seen.some(m => m.type === 'board_switched' && m.board === 'payments@option-a'));
  check('  so the source is off the screen, which is the failure this eval is about',
    (await paneShowing('payments')) === null,
    JSON.stringify((await api('GET', '/api/panes')).body?.panes?.map(p => p.board)));
  check('  though the board itself is unharmed, just not showing',
  ...
  check('  and it is the one about putting a proposal beside its source',
    /pane open/.test(mine?.expected_output ?? ''));
  check('  and it says what happens if the agent reuses the pane instead',
