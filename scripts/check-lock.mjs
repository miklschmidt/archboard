 ...
 */
const take = (request) =>
	holdBoard(request).catch((error) => ({ created: false, holder: null, error }));
...

{
	...
	let ran = 0;
	const answer = await withBoardLock({ board, holder: agent("first") }, () => {
		ran += 1;
		return "written";
	});
	...
	check(
		"a hold reports itself as the one that took it",
		held.created === true && held.holder?.id === "the-user",
		why(held),
	);

	const denied = await refusal(
		withBoardLock({ board, holder: agent("second"), waitMs: 120 }, () => {
			ran += 1;
		}),
	);
	check(
	...
	check(
		...
		/held by the person at the canvas, since \d\d:\d\d:\d\d \(\d+\.\d s\)/.test(
			denied?.message ?? "",
		),
		denied?.message,
	);
	check(
		"  and it waited rather than failing straight away",
		denied.waitedMs >= 100,
		`${denied?.waitedMs} ms`,
	);
	...
	check(
		'  and "since" stays when it was taken, not when it was last renewed',
		again.holder?.since === held.holder?.since,
		`${held.holder.since} -> ${again.holder.since}`,
	);
	check(
		"  and the lease moved forward",
		Date.parse(again.holder?.until) > Date.parse(held.holder?.until),
		why(again),
	);

	await withBoardLock({ board, holder: person("the-user") }, () => {
		ran += 1;
	});
	check(
		"a write by the holder runs, and does not release the hold it joined",
		ran === 2 && boardLockState(board)?.id === "the-user",
		`ran ${ran} / ${boardLockState(board)?.id}`,
	);
...

{
	...
	check(
		"a holder that never came back does not wedge the board",
	...
	check(
		"the lease clears the trailing idle deadline plus a write, with room",
		LOCK_LEASE_MS >= REPORT_IDLE_SETTLE_MS * 2,
		`idle ${REPORT_IDLE_SETTLE_MS} vs lease ${LOCK_LEASE_MS}`,
	);
	check(
		"an agent waiting on a crashed holder outlasts the lease rather than timing out first",
...

{
	...
	check(
		"an agent waits for a user hold rather than failing at it",
		got.created === true && waited >= 200 && waited < 1200,
		`${waited} ms, ${why(got)}`,
	);
...

{
	...
	check(
		"the lock is a file in the vault, where a second canvas can see it",
		fs.existsSync(file),
		file,
	);
	...
	check(
		"a lock file nothing can read does not wedge the board forever",
		after.created === true,
		why(after),
	);
...

{
	...
	check(
		"two spellings of one board are one lock (ADR 0010)",
		clash instanceof BoardHeldError,
		`${clash?.name}`,
	);
	...
	check(
		"a nested board name is one lock file rather than a directory to create",
...

{
	...
	onBoardLockChanged((board, holder) =>
		news.push({ board, held: holder !== null, id: holder?.id ?? null }),
	);
	const board = "broadcast";

	await take({ board, holder: person("a-pane"), waitMs: 0 });
	check(
		"taking a board is news immediately",
	...
	await take({ board, holder: person("a-pane"), waitMs: 0 });
	check(
		"  and renewing is not, because nothing a pane acts on changed",
		news.length === beforeRenew,
	);
	...
	check(
		"  it is news once the board has stayed free",
		news.at(-1)?.held === false,
		JSON.stringify(news.at(-1)),
	);
	...
	check(
		"eight writes in a row take a pane out of read-only once, at the end",
...

{
  ...
  `,
	...
	check(
		"  by a holder it can see is somewhere else",
		shut?.holder?.process === announced.process &&
			shut.holder.process !== `${os.hostname()}:${process.pid}`,
		`${shut?.holder?.process} vs ${os.hostname()}:${process.pid}`,
	);
	check(
		"  and the refusal says so rather than reading as a bug in this one",
		/on another canvas \(/.test(shut?.message ?? ""),
		shut?.message,
	);

	const eventually = await take({ board, holder: agent("this-canvas"), waitMs: 3000 });
	check(
		"and gets the board when the other one is done",
		eventually.created === true,
		why(eventually),
	);
...

{
	...
	check(
		"an agent can claim a board and say what it is doing",
		first.created === true && first.claim.holder.reason === "redrawing the payment path",
		JSON.stringify(first.claim),
	);
	check(
		"  and the claim is on the lock itself, not only in this process",
		boardLockState(board)?.claimed === true,
		JSON.stringify(boardLockState(board)),
	);

	const refused = await refusal(holdBoard({ board, holder: agent("somebody-else"), waitMs: 0 }));
	check(
		"  so a refusal says it is claimed rather than that somebody is mid-write",
		/held by an agent that has claimed it \(redrawing the payment path\)/.test(
			refused?.message ?? "",
		),
		refused?.message,
	);
	...
	for (let i = 0; i < 20; i += 1) {
		...
		const wrote = await withBoardLock(
			{ board, holder: { id: writer ?? `orphan-${i}`, kind: "agent" }, waitMs: 0 },
			() => true,
		).catch(() => false);
	...
	check(
		"twenty writes under one claim all go through",
		unwritten === 0,
		`${unwritten} of 20 refused`,
	);
	check(
		"  and leave no gap another writer could take, in any of the nineteen between them",
		gaps === 0,
		`${gaps} of 20 gaps`,
	);
	check(
		"  because every one of them is the claim writing, not a new hold",
		reacquired === 0,
		`${reacquired} writes wrote as somebody else`,
	);
	check(
		"  and the board was never let go and re-taken: one hold, since one moment",
	...
	check(
		"claiming again extends rather than starting a second claim",
		again.created === false && again.claim.holder.id === first.claim.holder.id,
		JSON.stringify(again.claim),
	);
	check(
		"  with the deadline moved and the reason brought up to date",
	...
	check(
		"a person takes a claimed board back rather than being refused",
		back.created === true && back.holder?.kind === "human",
		why(back),
	);
	check(
		"  and the claim is over on this canvas",
		claimOn(board) === null,
		JSON.stringify(claimOn(board)),
	);

	const told = takeClaimRevocation(board);
	check(
		"  and the agent is told it lost the board, and by whom",
		told?.by?.id === "a-user" && told?.claim?.holder?.reason === "now the queues",
		JSON.stringify(told),
	);
	check(
		"  once, because a permanent refusal would wedge the board against it",
...

{
	...
	const lockFileFor = (name) =>
		join(vault, ".archboard", "locks", `${encodeURIComponent(name)}.lock`);

	for (const [how, lapse] of [
		[
			"a lease whose time has passed",
			(file) => {
		...
		check(
			"  and the person takes the board",
			taken.created === true && taken.holder?.kind === "human",
			why(taken),
		);
		check(
			"  and the claim is over on this canvas",
		...
		const told = takeClaimRevocation(board);
		check(
			"  and the agent is still told it lost the board, and by whom",
...

{
	...
	check(
		"an agent takes the board for one write, unclaimed",
		writing.created === true && !writing.holder?.claimed,
		why(writing),
	);
	const waited = await refusal(
		holdBoard({ board, holder: person("a-user"), waitMs: 120, revokeClaim: true }),
	);
	check(
		"and a person waits that out rather than taking it",
		waited instanceof BoardHeldError && waited.holder?.id === "one-write",
		waited?.message,
	);
	check(
		"  and nothing is reported as a lost claim, because there was none",
		takeClaimRevocation(board) === null,
	);
...

{
	...
	while (Date.now() < firstLease + 500) {
		...
		if (!now || now.id !== idle.claim.holder.id) {
			lostAt = Date.now();
			break;
		}
		if (Date.parse(now.until) > seenUntil) {
			renewals += 1;
			seenUntil = Date.parse(now.until);
		}
		await sleep(100);
	}
	check(
		"and the board is held for every moment of the lease it was written with",
		lostAt === null,
		lostAt
			? `free ${lostAt - Date.parse(idle.claim.holder.since)} ms in, on a ${LOCK_LEASE_MS} ms lease`
			: "",
	);
	check(
		"  because the canvas kept moving the lease, the agent having done nothing",
	...
	check(
		"and giving it back frees the board",
...

{
	...
	check(
		"a claim is made with a deadline of its own",
		Date.parse(brief.claim.expires) > Date.now(),
		why(brief),
	);
	...
	check(
		"and when it runs out the board is free, with nobody having released it",
		boardLockState(board) === null,
		JSON.stringify(boardLockState(board)),
	);
	check("  and the canvas knows it is no longer holding one", claimOn(board) === null);
	check(
		"  and releasing an expired claim is not an error, it is tidying up late",
...
try {
	...
	check(
		"a pane is told where its board stands the moment it connects",
	...
	check(
		"a pane can take the board at the first change of a user edit",
		took.status === 200 && took.body?.holder?.kind === "human",
		JSON.stringify(took.body),
	);
	await sleep(150);
	check(
		"  and every pane on that board is told, before anybody touches it",
		heard.at(-1)?.held === true && heard.at(-1)?.holder?.id === PANE,
		JSON.stringify(heard.at(-1)),
	);
	...
	check(
		...
		(await api("POST", "/api/boards/hold?board=scratch", { clientId: PANE })).body?.created ===
			false,
	);
	...
	check(
		...
		waited >= LOCK_WAIT_CAP_MS - 200,
		`${waited} ms against a ${LOCK_WAIT_CAP_MS} ms cap`,
	);
	check(
		"  and it is told who has the board and since when",
		shut.body?.holder?.id === PANE &&
			/held by the person at the canvas, since/.test(shut.body?.error ?? ""),
		shut.body?.error,
	);
	...
	check(
		"  and the refusal carries the document a read returns after the wait",
	...
	check(
		"  with the board version current at that same refusal",
	...
	check(
		"and neither is opening one, which writes no note",
		opened.status === 200,
		`${opened.status}`,
	);
	...
	check(
		"  and the panes are told it is free",
		heard.at(-1)?.held === false,
		JSON.stringify(heard.at(-1)),
	);
	...
	const strangerHeld = await api("POST", "/api/boards/hold?board=scratch", {
		clientId: "another-pane",
	});
	...
	stopStranger();
	check(
		"  and a report from a different pane is refused while it does",
		mine.status === 409 && mine.body?.holder?.id === "another-pane",
		`${mine.status}`,
	);
	...
	check(
		"an agent claims a board over the API, saying what it is doing",
		claimed.status === 200 &&
	...
	await sleep(200);
	check(
		"  and every pane holding it is told who has it and why, before anybody touches it",
		heard.at(-1)?.held === true &&
			heard.at(-1)?.holder?.claimed === true &&
			heard.at(-1)?.holder?.reason === WHY,
		JSON.stringify(heard.at(-1)),
	);
	...
	for (let i = 0; i < 20; i += 1) {
		...
		const rival = await refusal(
			holdBoard({ board: "scratch", holder: agent(`rival-${i}`), waitMs: 0 }),
		);
		if (!(rival instanceof BoardHeldError)) gaps += 1;
	}
	...
	check(
		"  and none of them waited for the claim, because each of them was the claim",
		slowest < LOCK_WAIT_CAP_MS / 2,
		`slowest ${slowest} ms against a ${LOCK_WAIT_CAP_MS} ms wait cap`,
	);
	check(
		"  with no gap another writer could take, in any of the nineteen between them",
		gaps === 0,
		`${gaps} gaps`,
	);
	check(
		"  and the board was held once throughout, not taken and given back twenty times",
	...
	const denied = await api("POST", "/api/boards/claim?board=scratch", {
		reason: "carrying on regardless",
	});
	check(
	...
	check(
		"  and the current version beside it",
	...
	check(
		"  and every element written under the claim is still on the board",
		written === 20,
		`${written} of 20`,
	);
	...
	check(
		"releasing a claim somebody took back is tidying up late, not an error",
	...
	check(
		"a claim with no reason is refused: it is what the person is shown",
		unexplained.status === 400,
		`${unexplained.status}`,
	);
	const homeless = await api("POST", "/api/boards/claim", { reason: "anything" });
	check(
		"and one that names no board like every other call (ADR 0009)",
		homeless.status === 400,
		`${homeless.status}`,
	);
	...
	try {
		...
		const acrossServers = await fetch(
			`http://127.0.0.1:${OTHER_PORT}/api/elements?board=scratch&doing=writing+from+the+other+canvas`,
			{
				method: "POST",
		...
		check(
			...
			refused?.holder?.id === PANE,
			JSON.stringify(refused?.holder),
		);
		...
		check(
			"a pane on the second canvas starts out believing its board is free",
			overThere.at(-1)?.held === false,
			JSON.stringify(overThere.at(-1)),
		);

		const heardBefore = overThere.length;
		const elsewhere = await api("POST", "/api/boards/claim?board=scratch", {
			reason: "restructuring the queues",
		});
		...
		check(
			"  and the pane on the other canvas is told, with nobody having told it",
			news?.held === true &&
				news?.holder?.claimed === true &&
				news?.holder?.reason === "restructuring the queues",
			JSON.stringify(news),
		);
		check(
			"  before the touch rather than at the write: no write was made to find that out",
		...
		check(
			"  and the person at that canvas can take it back",
			reclaimed.status === 200,
			`${reclaimed.status}`,
		);
		...
		check(
			"  and the claim does not come back, now that the board is free again",
			overThere.at(-1)?.held === false,
			JSON.stringify(overThere.at(-1)),
		);
	...
	check(
		"a hold that names no board is refused like every other call (ADR 0009)",
		unnamed.status === 400,
		`${unnamed.status}`,
	);
	const anonymous = await api("POST", "/api/boards/hold?board=scratch", {});
	check(
		"and so is one that names no holder, because it could never be released",
...

{
	...
	const senders = sources.filter((file) =>
		/type:\s*['"`]board_lock['"`]/.test(fs.readFileSync(file, "utf-8")),
	);
	check(
	...
	check(
		"  and nothing else reaches into the lock directory",
		bypass.length === 0,
		bypass.map((f) => f.replace(repoRoot, "")).join(", "),
	);
}
...
console.log(
	`lock: ${checks} checks. One writer at a time, across two canvases over one vault, and the panes are told.`,
);
