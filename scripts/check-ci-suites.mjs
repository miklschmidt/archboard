...
//
// The workflow used to name two scripts of its own, `check-mcp-stdio.mjs` and
// `check-local-bind.mjs`, and the suite grew to sixteen around it. Both were
...
const suites = Object.keys(pkg.scripts).filter(name => /^test:/.test(name));
...
for (const suite of suites) {
  if (chain.has(suite) && suite in SKIPPED) {
    ...
    fail(`\`${suite}\` runs on nobody's push. Add it to the \`test\` script in ` +
...
for (const named of workflow.match(/bun run test:[\w-]+/g) ?? []) {
  ...
  if (!(suite in SKIPPED)) {
    fail(`the workflow runs \`${suite}\` by name, and the chain already runs it. ` +
