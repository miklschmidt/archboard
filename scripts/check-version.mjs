...
// that lives long enough to remember. The gap
...

{
	...
	check(
		"the first write archboard makes to a note starts the count at 1",
		first.version === 1,
		String(first.version),
	);
	check(
		"  written into the frontmatter as a plain key, beside board, variant and level",
	...
	check(
		"  and only that line moved: the version is not stamped anywhere else",
	...
	check(
		"  and the same answer comes off the head of the file, without reading the scene",
...

{
	...
	check(
		"writing the same board again leaves the note byte-identical",
		before.equals(after),
		`${before.length} then ${after.length}`,
	);
	check(
		"  so the version does not move, and two saves still produce one document",
		again.version === 2,
...

{
	...
	check(
		"a version stated for this write beats the older one the canvas remembers",
		checkBoardVersion({
			board: ledgerKey,
			file: ledger.file,
			writesNote: true,
	...
	check(
		"without a stated version, the remembered one is the write precondition",
		conflict?.expected === 1 && conflict.actual === 2,
		JSON.stringify({ expected: conflict?.expected, actual: conflict?.actual }),
	);
	check(
		"a refusal remembers the version it just told that writer",
		rememberedVersion(writer) === 2,
		String(rememberedVersion(writer)),
	);

	forgetRememberedVersion(writer);
	check(
		"the note's own current version is deliberately not an expectation source",
...

{
	...
	const theirs = fs
		.readFileSync(board.file, "utf-8")
		.replace(/^version: 1$/m, "version: second draft");
	...
	check(
		"a `version` key holding something that is not a count is left exactly as it is",
		/^version: second draft$/m.test(note),
		note.split("\n").find((l) => l.startsWith("version")),
	);
	check(
		"  so the board is simply unversioned rather than having its frontmatter overwritten",
		written.version === null && versionNumber(note) === null,
		String(written.version),
	);
	check(
		"  and the write went through, because the hash is what guards a note and still did",
...

{
	...
	check(
		"a foreign write is named rather than inferred: the version stood still and the bytes moved",
		foreign?.versionMove === "unchanged" &&
			foreign.expectedVersion === 2 &&
			foreign.actualVersion === 2,
		JSON.stringify({
			move: foreign?.versionMove,
			ours: foreign?.expectedVersion,
			theirs: foreign?.actualVersion,
		}),
	);
	...
	check(
		"  and the refusal says which side is ahead, not only that the two differ",
		refusal?.versionMove === "unchanged" && /does not keep that mark/.test(refusal?.message ?? ""),
		refusal?.message?.split("\n")[2],
	);
	check(
		"  with both numbers as data, so a surface can say it in its own words",
	...
	check(
		"a note that has gone backwards is a revert, which no equality check can see",
	...
	check(
		"a note another archboard has written is ahead, and says by how many writes",
	...
	check(
		"a side with no number to order by says so rather than guessing",
...

{
	...
	check(
		"the mark on a board somebody else wrote now says which side is ahead",
	...
	check(
		"  in the same sentence the refusal uses, because there is one comparison",
	...
	check(
		"  and over a foreign writer it says that instead, which is a different answer",
...
const cli = (args) =>
	spawnSync(process.execPath, [src("bin.ts"), ...args], {
		encoding: "utf-8",
		input: "",
		env: {
			...process.env,
...
try {
	...
	check(
		"a write answers with the version it produced, so a writer never has to ask",
		first.body?.fingerprint?.version === 1,
		JSON.stringify(first.body?.fingerprint),
	);
	const noteFile = join(serverVault, "payments.excalidraw.md");
	check(
		"  and that is the version in the vault, not the one before the write",
		versionOfNoteAt(noteFile) === 1,
		String(versionOfNoteAt(noteFile)),
	);
	check(
		"  and the hash beside it is the hash of the bytes on disk",
	...
	check(
		"the next write moves it on by one",
		second.body?.fingerprint?.version === 2,
		JSON.stringify(second.body?.fingerprint),
	);

	const info = await api("GET", "/api/boards/info?board=payments");
	check(
		"and a board says which version it is at without being written to",
		info.body?.version === 2,
		JSON.stringify(info.body?.version),
	);
	...
	check(
		"a write against the version the board is at goes through",
	...
	check(
		"a write against a version the board has moved past is refused",
		stale.status === 409 && stale.body?.code === "BOARD_VERSION_CONFLICT",
		`${stale.status} ${stale.body?.code}`,
	);
	check(
		"  naming both versions, so the writer knows what it was and what it is",
		stale.body?.versionConflict?.expected === 2 && stale.body?.versionConflict?.actual === 3,
		JSON.stringify({
			expected: stale.body?.versionConflict?.expected,
			actual: stale.body?.versionConflict?.actual,
		}),
	);
	check(
		"  and saying by how many writes somebody else got there first",
		/1 time\(s\)/.test(stale.body?.error ?? ""),
		stale.body?.error?.split("\n")[1],
	);
	const staleRead = await api("GET", "/api/elements?board=payments");
	const staleInfo = await api("GET", "/api/boards/info?board=payments");
	check(
		"  with the current document in the refusal, not a second read left to make",
	...
	check(
		"  and the current version beside that document",
	...
	check(
		"  and the board left saving, because a precondition is not a conflict on the note",
		(await api("GET", "/api/boards/info?board=payments")).body?.held === undefined,
	);

	const nonsense = await api(
		"POST",
		"/api/elements?board=payments&expectVersion=soon",
		box("five", 800),
	);
	check(
		"an expectation that is not a number is refused rather than dropped",
	...
	check(
		"a board with no note yet is at no version, and a writer that says so is right",
	...
	check(
		"the precondition is on the write boundary, so it covers a route it was not written for",
		batch.status === 409 && batch.body?.code === "BOARD_VERSION_CONFLICT",
		`${batch.status}`,
	);
	...
	const claim = await api("POST", "/api/boards/claim", {
		board: "claimed",
		reason: "redrawing the claimed board",
	});
	check(
		"taking a board tells the agent what version it is at, so the count starts somewhere",
		claim.body?.version === 1,
		JSON.stringify(claim.body?.version),
	);

	const underClaim = await api("POST", "/api/elements?board=claimed", box("eleven", 200));
	check(
		"a write under the claim is checked against that, and it matches",
	...
	fs.writeFileSync(
		claimedFile,
		mine.replace(/^version: 2$/m, "version: 4").replace('"x": 200', '"x": 260'),
	);

	const carriedNothing = await api("POST", "/api/elements?board=claimed", box("twelve", 400));
	check(
		"and a write after somebody else got there is refused, with the caller saying nothing at all",
		carriedNothing.status === 409 && carriedNothing.body?.code === "BOARD_VERSION_CONFLICT",
		`${carriedNothing.status} ${carriedNothing.body?.code}`,
	);
	check(
		"  naming what this writer was working from and what the board is at",
		carriedNothing.body?.versionConflict?.expected === 2 &&
			carriedNothing.body?.versionConflict?.actual === 4,
		JSON.stringify({
			was: carriedNothing.body?.versionConflict?.expected,
			is: carriedNothing.body?.versionConflict?.actual,
		}),
	);
	check(
		"  and telling it once: the refusal is itself a telling, so it is not wedged on one stale read",
	...
	check(
		"  its next write goes against what it was just told",
		after.status === 409 || after.status === 200,
		`${after.status}`,
	);
	check(
		"  and by then it is the hash refusing, because those really were somebody else's bytes",
		after.body?.conflict?.reason === "changed",
		String(after.body?.conflict?.reason),
	);
	...
	setWriteDoing("driving a long-lived client");
	...
	check(
		"a long-lived client is told the version by the write it just made",
		firstCall.fingerprint?.version === 1,
		JSON.stringify(firstCall.fingerprint),
	);

	const secondCall = await applyElementChanges({ upserts: [box("r2", 200)] });
	check(
		"  and its next write is checked against it without the agent naming one",
		secondCall.fingerprint?.version === 2,
		JSON.stringify(secondCall.fingerprint),
	);

	const theirs = fs.readFileSync(rememberedFile, "utf-8");
	fs.writeFileSync(
		rememberedFile,
		theirs.replace(/^version: 2$/m, "version: 6").replace('"x": 200', '"x": 280'),
	);
	...
	check(
		"  and it learns the real version from the refusal, so it is not stuck repeating it",
		client.currentExpectedVersion() === 6,
		String(client.currentExpectedVersion()),
	);

	setExpectedVersion(2);
	check(
		"a version the caller states beats the one the client remembers",
		client.currentExpectedVersion() === 2,
		String(client.currentExpectedVersion()),
	);
	setExpectedVersion(null);
	forgetVersionsSeen();
	...
	const person = await fetch(`${base}/api/elements/changes?board=payments&expectVersion=1`, {
		...
		body: JSON.stringify({
			upserts: [box("theirs", 700)],
			deletes: [],
			clientId: "pane-1-somebody",
		}),
	});
	check(
		"a person's change report is never version-refused, even carrying a stale one",
		person.status === 200,
		String(person.status),
	);
	check(
		"  and is not made to say what it is doing either, which is the same line drawn twice",
	...
	check(
		"a foreign edit leaves the version exactly where archboard left it",
		versionOfNoteAt(sharedFile) === 1,
		String(versionOfNoteAt(sharedFile)),
	);
	...
	check(
		"so a write whose expectation passes is refused all the same, by the hash",
	...
	check(
		"  and the refusal names the foreign writer, which is what the version added to it",
	...
	check(
		"  with their note untouched: nothing was written over it",
