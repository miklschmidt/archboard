...
try {
	...
	const rebound = {
		...arrow0,
		startBinding: { elementId: "boxC", focus: 0, gap: 4, fixedPoint: null },
		points: [
			[0, 0],
			[300, -270],
		],
	};
	await api("POST", "/api/elements/changes?board=probe", {
		upserts: [rebound],
		deletes: [],
		clientId: "probe-human",
	});

	const mid = (await api("GET", "/api/elements?board=probe")).body.elements.find(
		(e) => e.id === "arr",
	);
	...
	const after = (await api("GET", "/api/elements?board=probe")).body.elements.find(
		(e) => e.id === "arr",
	);
	...
	console.log(
		"\nVERDICT:",
		pointsAfterRebind === JSON.stringify(after.points)
