...

{
	...
	check(
		"flat top-level metadata stays a plain element in describe",
		/0 nodes/.test(description) &&
			/1 plain/.test(description) &&
			/customData: kind=service/.test(description),
		description.replace(/\n/g, " | "),
	);
	...
	check(
		"  and compare classifies the same element as plain foreign customData",
		comparison.to.nodeCount === 0 &&
			comparison.to.plainCount === 1 &&
	...
	check(
		"  and the changes engine treats it as an anonymous drawing, not a promoted service",
...
{
	const base = scene();
	const nudged = base.map((el) =>
		el.id === "b" ? { ...el, x: 312 } : el.id === "bl" ? { ...el, x: 332 } : el,
	);
	const change = diff(base, nudged);
	check(
		"a 12px nudge is not a change worth reporting",
...

{
	...
	check(
		"dragging a node out of its cluster is a layout change",
		change.significance === "layout",
		change.significance,
	);
	const moved = change.nodes.moved.find((m) => m.node === "auth");
	check(
		"  and the node is reported as leaving the company it kept",
		Boolean(moved) && "cluster" in moved.changes,
	);
	check(
		"  and the narration names nodes, never node ids",
...

{
	...
	const change = diff(
		base,
		base.filter((el) => el.id !== "e1"),
	);
	check(
		"cutting an edge is structural",
		change.significance === "structural" && change.counts.edgesRemoved === 1,
	);
	check(
		"  and it is named by both ends",
		change.edges.removed[0]?.fromName === "Gateway" &&
			change.edges.removed[0]?.toName === "AuthService",
	);
}

{
	const base = scene();
	const rerouted = base.map((el) =>
		el.id === "e2" ? { ...el, startBinding: { elementId: "a" } } : el,
	);
...

{
	...
	check(
		"a box the human drew but nobody promoted is still reported",
		change.significance === "structural" && change.counts.nodesAdded === 1,
	);
	check(
		"  and it is described, not identified by id",
		change.nodes.added[0]?.anonymous === true && change.nodes.added[0]?.name === "Redis",
	);

	const promoted = base.map((el) =>
		el.id === "z"
			? { ...el, customData: { archboard: { node: "redis", kind: "datastore", name: "Redis" } } }
			: el,
	);
	const after = diff(base, promoted);
	check(
		"promoting it reads as a promotion, not as a deletion plus an addition",
		after.counts.identityChanges === 1 &&
			after.counts.nodesAdded === 0 &&
			after.counts.nodesRemoved === 0,
		JSON.stringify(after.counts),
	);
	check(
		"  and it does not make the cluster look like it churned",
		after.layout.clusters.length === 0,
		JSON.stringify(after.layout.clusters.map((c) => c.kind)),
	);
	check(
		"  and the promotion knows what it used to be",
		after.nodes.identity[0]?.what === "promoted" &&
			after.nodes.identity[0]?.to.kind === "datastore",
	);
}

{
	...
	check(
		"a node gaining its bound label on first sync is not a semantic change",
		change.counts.nodesChanged === 0,
		JSON.stringify(change.counts),
	);
}

{
	...
	check(
		"recolouring is cosmetic, and cosmetic is not an event",
...

{
	...
	check("  while a label that came along is not", boundTextDrift(carried).length === 0);

	const wrong = diff(base, stranded);
	const right = diff(base, carried);
	check(
		"a dragged node leaves the cluster it was in",
		right.nodes.moved.length === 3 && right.nodes.moved.every((node) => "cluster" in node.changes),
	);
	check(
		"  and with its label left behind, the same drag reports none of that",
	...
	check(
		"  it reports the node growing instead, which is not what happened",
...

{
	...
	for (const [name, elements] of Object.entries(boards)) {
		...
		check(
			`${name}: every bound label sits on the thing it names`,
			drifted.length === 0,
...
// once it reaches a thread as prose.
{
	...
	check(
		"a node added past the right edge does not move the nodes nobody touched",
		regionMoves(added).length === 0,
		regionMoves(added)
			.map((m) => `${m.name} ${JSON.stringify(m.changes.region)}`)
			.join("; "),
	);
	check(
		"  and the arriving node is still placed at the right of the frame it arrived in",
		added.detail.nodes.added[0]?.layout.region === "top-right",
		added.detail.nodes.added[0]?.layout.region,
	);
	check(
		"  and the frame is the box round the nodes both sides have, not round everything",
	...
	const removed = diff(
		spread,
		spread.filter((el) => el.id !== "c"),
	);
	check(
		"a node deleted from the right edge does not move the nodes nobody touched",
	...
	check(
		"a node genuinely dragged across the board still reports its new region",
		regionMoves(genuine)
			.map((m) => m.node)
			.join() === "bravo",
		regionMoves(genuine)
	...
	check(
		"  and it reports where it went",
		genuine.nodes.moved[0]?.changes.region?.to === "middle-right",
		JSON.stringify(genuine.nodes.moved[0]?.changes.region),
	);
	check(
		"  while the nodes it was dragged past are not called moved",
		regionMoves(genuine).every((m) => m.node === "bravo"),
	);

	check(
		"panning the whole board reports nothing at all",
		diff(
			spread,
			spread.map((el) => ({ ...el, x: el.x + 5000, y: el.y + 5000 })),
		).significance === "none",
	);
...

{
	...
	check(
		"flat top-level metadata reaches the feed as an anonymous drawing, not a promoted service",
...

{
	...
	check(
		"a drag of 30 element updates settles into exactly one event",
		event !== null && event.mutations === 30,
		event ? `mutations=${event.mutations}` : "no event",
	);
	check("  attributed to the human, because the browser reported it", event?.origin === "human");
	check(
		"  and a second settle with nothing pending says nothing",
		changeFeed.settle("payments") === null,
	);
	...
	check(
		"the next real change still becomes an event",
		next !== null && next.cursor === cursorBefore + 1,
	);
	...
	// wrong answer and so is the hardest kind to notice.
	{
		...
		check(
			"customData edited in place is still a change, so the baseline is a copy",
			kindEvent !== null,
		...
		check(
			"  and the copy shares neither customData nor boundElements",
	...
	check(
		"a caller can ask for one net diff since a cursor rather than a replay",
		net !== null && net.events.length === 2 && net.change.significance !== "none",
		net ? `${net.events.length} events` : "no checkpoint",
	);
	check(
		"a cursor the feed can no longer reach answers null, not a wrong diff",
...

{
	...
	check(
		"a board arriving wholesale sets the baseline instead of emitting an event",
...
wss.on("connection", (ws) => {
	...
	ws.on("message", (raw) => {
		...
		if (!initialized) {
			...
			ws.send(
				msg.params.expectedTurnId === "turn-1"
...

{
	...
	check(
		"injection refuses to arm when the canvas is not on loopback",
	...
	check(
		"being on loopback is not enough on its own — the switch is required",
		injection.injectionStatus().armed === false,
	);
	let refused = false;
	await injection.injectTest("nope").catch(() => {
		refused = true;
	});
...

{
	...
	delete process.env.ARCHBOARD_INJECT_THREAD;
	injection.startInjection("127.0.0.1");
	let status = injection.injectionStatus();
	check(
		"injection refuses an enabled canvas without an explicit task route",
		status.armed === false && /ARCHBOARD_INJECT_THREAD/.test(status.refusal ?? ""),
		status.refusal ?? "",
	);
	injection.stopInjection();

	process.env.ARCHBOARD_INJECT_THREAD = "thread-live";
	injection.startInjection("127.0.0.1");
	await sleep(300);
	status = injection.injectionStatus();
	check(
		"armed on loopback with the switch and explicit task route set",
		status.armed && status.connected,
		status.lastError ?? "",
	);
	check(
		"the target is exactly the configured task",
		status.target.threadId === "thread-live" && status.target.reason === "pinned",
	...
	check(
		"  and appends a developer message in the shape the app-server expects",
	...
	check(
		"  and never sends a jsonrpc field, which this protocol does not use",
	...
	check(
		"loud can be forced for one probe, and steers the running turn",
	...
	await sleep(400);
	check(
		"the agent's own changes are never injected back at it",
		injection.injectionStatus().injected.quiet === before,
	);
	check(
		"  and saying what it was doing does not become a way round that",
		injection.injectionStatus().injected.quiet === before,
	);

	// A human's change is.
	elements = elements.map((el) =>
		el.id === "b" ? { ...el, y: 1600 } : el.id === "bl" ? { ...el, y: 1640 } : el,
	);
	...
	check(
		"a human's change is injected, once",
		after.injected.quiet === before + 1,
		`${before} -> ${after.injected.quiet}`,
	);
	check(
		"  and what the thread receives names the board and says nothing is being asked of it",
		/\[archboard\]/.test(after.lastInjection.text) &&
			/Nobody is waiting on you/.test(after.lastInjection.text),
	);
	check(
		"  carrying what an agent said it was doing, as context for the person's act",
		/An agent was at: rerouting orders through the queue/.test(after.lastInjection.text),
		after.lastInjection.text.split("\n").find((line) => /An agent was at/.test(line)) ??
			"(not there)",
	);
