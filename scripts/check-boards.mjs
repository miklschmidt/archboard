...
// the same: the note keeps the name a human typed.
{
	...
	check(
		"  and two spellings are one address",
		boardKey(parseBoardKey("Payments")) === boardKey(parseBoardKey("payments")),
	);
	check(
		"  and the casing that was typed is kept for showing",
		boardDisplayName(parseBoardKey("Payments")) === "Payments",
	);
	check(
		"  and a name already lowercase carries nothing extra",
		parseBoardKey("payments").displayName === undefined,
	);
	check(
		"a variant is a slug, so it is lowercased outright",
		boardKey(parseBoardKey("payments@Option-A")) === "payments@option-a",
	);
	check(
		"a nested name is normalised segment by segment",
	...
	check(
		"an accented name is one address however it is composed",
		normalizeBoardKey(nfd) === normalizeBoardKey(nfc) &&
			boardKey(parseBoardKey(nfd)) === boardKey(parseBoardKey(nfc)),
	);

	check(
		"the frontmatter records the casing a human chose, not the key",
		identityFrontmatter(makeIdentity({ board: "Payments" })).find(([k]) => k === "board")?.[1] ===
			"Payments",
	);
	...
	try {
		fs.writeFileSync(
			path.join(scratch, "Payments.excalidraw.md"),
			'---\nboard: Payments\nvariant: current\n---\n\n# Excalidraw Data\n\n## Text Elements\n\n%%\n## Drawing\n```json\n{"type":"excalidraw","version":2,"elements":[],"appState":{}}\n```\n%%\n',
		);
		check(
			"a note already in the vault is found under its own casing",
			vaultPathFor(parseBoardKey("payments"), scratch) ===
				path.join(scratch, "Payments.excalidraw.md"),
		);
		check(
			"  and reading it reports that casing, not the address that was typed",
			readBoardFile(parseBoardKey("payments"), scratch)?.identity?.displayName === "Payments",
		);
		check(
			"  and it is not reported as declaring a different board",
			readBoardFile(parseBoardKey("payments"), scratch)?.declaredKey === undefined,
		);
		check(
			"a note that does not exist yet takes the casing that was asked for",
			vaultPathFor(parseBoardKey("NewBoard"), scratch) ===
				path.join(scratch, "NewBoard.excalidraw.md"),
		);
		...
		fs.writeFileSync(
			path.join(scratch, `${nfd}.excalidraw.md`),
			'---\nboard: café\nvariant: current\n---\n\n# Excalidraw Data\n\n%%\n## Drawing\n```json\n{"type":"excalidraw","version":2,"elements":[],"appState":{}}\n```\n%%\n',
		);
		check(
			"a decomposed filename is found by its composed spelling",
		...
		check(
			"two notes at one address are both listed",
			listed.filter((b) => b.key === "payments").length === 2,
		);
		check(
			"  and each one names the others it collides with",
			listed.filter((b) => b.key === "payments").every((b) => b.collidesWith?.length === 1),
		);
		check(
			"  and which one is reachable does not depend on readdir order",
			vaultPathFor(parseBoardKey("payments"), scratch) ===
				vaultPathFor(parseBoardKey("PAYMENTS"), scratch),
		);
	} finally {
...

{
	let refused = null;
	try {
		resolveBoard(undefined, "Adding an element");
	} catch (error) {
		refused = error;
	}
	...
	check(
		"  and lists what is open, so the next step is on screen",
		(refused?.open ?? []).includes(SCRATCH_KEY),
	);
	check("  and it is a client error, not a server fault", refused?.status === 400);

	check(
		"the scratch board exists from boot, so a first run has something to name",
	...
	check(
		"a board that is not open is a different refusal",
		/is not open/.test(unopened?.message ?? ""),
	);
...
// every open board a second copy that could drift from the note.
{
	...
	check(
		"  and what it does hold is where the note is and what was last seen there",
...
// server below.
{
	...
	check(
		"  holding what the note held",
		Boolean(kept) && kept.x === 0 && kept.customData.archboard.kind === "gateway",
	);
	...
	check(
		"changing the board after snapshotting leaves the snapshot unchanged",
		kept.x === 0 && kept.customData.archboard.kind === "gateway",
		JSON.stringify(kept),
	);
	check(
		"  and the change really did land on the board",
...
// note must remain byte-identical.
{
	...
	check(
		"the exact final document is geometry-checked after write settlement",
	...
	check(
		"  before the persistence boundary changes one byte",
...
// mean one pane to `--pane` and another to `panes`.
{
	...
	check(
		"panes are ordered left to right, whatever order they registered in",
		panesInOrder(two)
			.map((entry) => entry.pane.clientId)
			.join(",") === "a,b",
	);
	...
	check(
		"--pane primary is the one that answers for the browser",
		resolvePaneSpec(two, "primary").clientId === "a",
	);

	let unknown = null;
	try {
		resolvePaneSpec(two, "middle");
	} catch (error) {
		unknown = error;
	}
	check(
		"an unknown pane is refused, and the message lists the real ones",
		/No pane called "middle"/.test(unknown?.message ?? "") &&
			/payments@option-a/.test(unknown?.message ?? ""),
	);
	check(
		"  and with the screen full it does not offer to make another",
		!/pane open/.test(unknown?.message ?? ""),
	);

	let missing = null;
	try {
		resolvePaneSpec([two[1]], "right");
	} catch (error) {
		missing = error;
	}
	check(
		"a pane that could exist but does not says how to make it (TASK-033)",
		/archboard pane open/.test(missing?.message ?? ""),
		missing?.message,
	);
	...
	// the same and silence that.
	{
		...
		const stamped =
			plan.nodes[0]?.customData?.archboard ?? plan.updates?.[0]?.customData?.archboard ?? {};
		check(
			"a promoted node takes its board's variant",
			stamped.variant === "option-a",
			JSON.stringify(stamped),
		);
		check(
			"  and no level, because none was asked for (ADR 0013)",
			stamped.level === undefined,
			`level=${stamped.level}`,
		);
		...
		const explicit =
			withLevel.nodes[0]?.customData?.archboard ??
			withLevel.updates?.[0]?.customData?.archboard ??
			{};
		check(
			"  and records one when the caller says the node differs from its board",
			explicit.level === "service",
	...
	// them. They drifted once already, so this asserts they agree (TASK-050).
	{
		...
		check(
			...
			named.every((word) => specs.includes(word)),
			specs,
		);
		...
		let gone = null;
		try {
			resolvePaneSpec([two[1]], "only");
		} catch (error) {
			gone = error;
		}
		check(
			"  and a spelling nobody documents is refused rather than quietly working",
			/No pane called "only"/.test(gone?.message ?? ""),
			gone?.message ?? "it resolved",
		);
		check(
			"  with the refusal naming what does work",
	...
	check(
		"with two panes, which one is not guessed at",
...
try {
	...
	check(
		"  and names the exact field path that failed",
		/viewport\.x/.test(invalidPane.body?.error ?? ""),
		invalidPane.body?.error,
	);
	...
	check(
		"a malformed legacy note is refused through board open",
		legacyOpen.status === 400,
		legacyOpen.body?.error,
	);
	check(
		"  with the element, type and every invalid field named",
		/helv \(text\): width, height/.test(legacyOpen.body?.error ?? ""),
		legacyOpen.body?.error,
	);
	check(
		"  without rewriting the malformed note",
		fs.readFileSync(legacyFile, "utf8") === legacyNote,
	);
	const afterLegacyRefusal = await api("GET", "/api/boards");
	check(
		"  and without registering an empty stand-in board",
	...
	check(
		"a valid geometry fixture is persisted",
		seededGeometry.status === 200,
	...
	check(
		"a mixed valid and malformed agent batch is refused",
		mixedGeometry.status === 400,
		mixedGeometry.body?.error,
	);
	check(
		"  and identifies the malformed element and both fields",
		/helvetica \(text\): width, height/.test(mixedGeometry.body?.error ?? ""),
		mixedGeometry.body?.error,
	);
	check(
		"  without changing one byte of the note",
	...
	check(
		"  or applying the valid half in memory",
	...
	check(
		"the human change path refuses the same malformed geometry",
		humanGeometry.status === 400,
		humanGeometry.body?.error,
	);
	check(
		"  and leaves the note byte-identical too",
		fs.readFileSync(geometryFile).equals(geometryBefore),
	);

	const left = await openPane("p-left", 0, { primary: true, focused: true });
	check(
		"a fresh pane holds the scratch board, so there is something to name",
		left.board() === "scratch",
	);

	const opened = await api("POST", "/api/boards/open", { board: "payments" });
	check(
		"with one pane, a board opens into it without being told which",
	...
	check(
		"with two panes, opening a board without naming one is refused",
		unaddressed.status === 400,
		`got ${unaddressed.status}`,
	);
	check(
		"  and the message names the panes",
		/--pane left \| right/.test(unaddressed.body?.error ?? ""),
	);
	...
	const unaddressedNew = await api("POST", "/api/boards/new", {
		board: "never-made",
		level: "service",
	});
	check(
		"with two panes, starting a board without naming one is refused as well",
		unaddressedNew.status === 400,
		`got ${unaddressedNew.status}`,
	);
	const afterRefusal = await api("GET", "/api/boards");
	check(
		"  and the board it refused to start does not exist",
		!(afterRefusal.body?.open ?? []).some((entry) => entry.key === "never-made"),
		(afterRefusal.body?.open ?? []).map((entry) => entry.key).join(","),
	);
	check(
		"  and no note was written for it either",
		!fs.existsSync(path.join(vault, "never-made.excalidraw.md")),
	);

	const takenName = await api("POST", "/api/boards/new", { board: "payments" });
	check(
		"a name already taken is reported as taken, whatever the panes are doing",
	...
	check(
		"and a board that is nowhere says so, rather than asking which pane to put it in",
		openMissing.status === 404 && /No board "never-made"/.test(openMissing.body?.error ?? ""),
		openMissing.body?.error,
	);

	const leftBefore = left.since();
	const intoRight = await api("POST", "/api/boards/open", {
	...
	check(
		"the switch reached the pane it was addressed to",
		right.seen.some((m) => m.type === "board_switched" && m.board === "payments@option-a"),
	);
	check(
		"and reached no other pane",
	...
	check(
		"  naming both",
		panes.body?.panes?.map((p) => p.board).join(",") === "payments,payments@option-a",
	);
	check(
		"  and drops the line saying the server holds one board at a time",
		!/one board at a time/.test(panes.body?.text ?? ""),
	);
	check(
		"  and says what an unnamed call will do instead",
	...
	check(
		"the same element with a board lands",
		qualified.status === 201 || qualified.status === 200,
	);
	const after = await api("GET", "/api/elements?board=payments");
	check(
		"  on that board and no other",
	...
	check(
		"clearing without a board is refused too — the dangerous one most of all",
	...
	check(
		"a pick belongs to the pane that made it",
	...
	check(
		"clearing one board leaves the other pane's pick alone",
	...
	fs.writeFileSync(
		savedA.body.file,
		fs.readFileSync(savedA.body.file, "utf-8") + "\nedited elsewhere\n",
	);
	const refusedSave = await api("POST", "/api/boards/save?board=payments@option-a");
	check(
		"a note that changed underneath is still refused, per board (ADR 0006)",
	...
	check(
		"  and the pane still open keeps its own board",
	...
	check(
		"with one pane, the report says how to get a second",
		/archboard pane open/.test(lonely.body?.text ?? ""),
		lonely.body?.text,
	);

	const noSuchPane = await api("POST", "/api/boards/open", {
		board: "payments@option-a",
		pane: "right",
	});
	check("opening into a pane that does not exist is still refused", noSuchPane.status === 400);
	check(
		"  and the refusal now says how to make it, not just that it is missing",
		/archboard pane open/.test(noSuchPane.body?.error ?? ""),
		noSuchPane.body?.error,
	);

	const closingTheLast = await api("POST", "/api/panes/close", { pane: "1" });
	check("the only pane cannot be closed", closingTheLast.status === 409);
	check(
		"  and the refusal says the board is unaffected either way",
	...
	check(
		"a pane can be opened with no browser interaction",
		split.status === 200,
		split.body?.error,
	);
	check(
		"  and the answer says where it landed",
		split.body?.pane?.place === "right",
		JSON.stringify(split.body?.pane),
	);
	...
	check(
		"  and the refusal names what is on screen and what to do instead",
	...
	check(
		"  and the refusal spells out both options",
		/pane close left/.test(unnamed.body?.error ?? "") &&
			/pane close right/.test(unnamed.body?.error ?? ""),
		unnamed.body?.error,
	);
	...
	check(
		"  and it moved there",
		second.seen.slice(secondBeforeCamera).some((m) => m.type === "set_viewport"),
	);
	check(
		"  and nowhere else",
		left.seen.slice(leftBeforeCamera).every((m) => m.type !== "set_viewport"),
	);

	const badCamera = await api("POST", "/api/viewport", { scrollToContent: true, pane: "middle" });
	check(
		"a camera aimed at no pane is refused rather than sent somewhere",
	...
	const intoTheRight = await api(
		"POST",
		"/api/elements/from-mermaid?board=payments@option-a",
		diagram,
	);
	check(
		"mermaid converts in the pane holding the board it was given",
		intoTheRight.status === 200,
		intoTheRight.body?.error,
	);
	check(
		"  which here is the right one, and no --pane was passed or exists",
		intoTheRight.body?.pane?.place === "right",
		JSON.stringify(intoTheRight.body?.pane),
	);
	check(
		"  and that is not the pane that answers for the browser",
	...
	check(
		"  carrying the board that was asked for",
		arrived?.board === "payments@option-a",
		arrived?.board,
	);
	check(
		"  and the pane holding the current architecture was left alone",
	...
	check(
		"a board no pane is holding is refused, not converted somewhere else",
		notShowing.status === 409,
		JSON.stringify(notShowing.body),
	);
	check(
		"  with nothing sent to either pane",
	...
	check(
		"  and the board it was showing is still open, just not on screen",
	...
	const roomForOne = await api(
		"POST",
		"/api/elements/from-mermaid?board=payments@option-a",
		diagram,
	);
	check(
		"the board just taken off screen can no longer be converted into",
		roomForOne.status === 409,
		JSON.stringify(roomForOne.body),
	);
	check(
		"  and with display room it offers a new pane, not a repointed one",
	...
	check(
		"  and the caller is told which kind of problem it is",
	...
	check(
		"and so does mermaid, which has no pane to convert in either",
		headlessMermaid.body?.code === "BROWSER_REQUIRED",
		JSON.stringify(headlessMermaid.body),
	);
	...
	for (const args of [
		...
		check(
			`\`${args.join(" ")}\` exits 4 when no browser is open`,
			run.code === 4,
			`exit ${run.code}`,
		);
	...
	const branched = await api("POST", "/api/boards/save?board=ledger", {
		name: "ledger",
		variant: "option-a",
	});
	check(
		"save --as branches the board",
		branched.status === 200 && branched.body?.board === "ledger@option-a",
	);
	const saveChanges = await api("GET", "/api/changes?board=ledger@option-a&since=0");
	check(
		"  and the change feed reports the write to the named destination",
	...
	check(
		"  and every node on the copy records the variant it was saved as",
	...
	check(
		"  while the board it was branched from is untouched",
		variantsOf(onOrigin.body).every((v) => v === "current"),
		variantsOf(onOrigin.body).join(","),
	);

	const branchNote = fs.readFileSync(branched.body.file, "utf-8");
	check(
		"  including in the note on disk, which is what compare reads",
	...
	check(
		"  and the branch is at the level its source was at",
		branched.body?.identity?.level === "service",
		JSON.stringify(branched.body?.identity),
	);
	check(
		"  including in the note, which is where identity is recorded",
		/^level: service$/m.test(branchNote),
	);
	const levelled = await api("POST", "/api/boards/save?board=ledger", {
		name: "ledger@option-d",
		level: "module",
	});
	check(
		"  while a level the caller states still wins over the source's",
		levelled.body?.identity?.level === "module",
		JSON.stringify(levelled.body?.identity),
	);
	...
	check(
		"the diff reports the one node the human removed",
		diff.body?.summary?.nodesRemoved === 1 && diff.body?.nodes?.removed?.[0]?.node === "worker",
	);
	check(
		"  and nothing else as changed",
		diff.body?.summary?.nodesChanged === 0,
	...
	check(
		"  with no stale-variant warning to explain away",
	...
	await api(
		"POST",
		"/api/elements?board=billing@option-b",
		promoted("gw", "Gateway", 0, "gateway"),
	);
	...
	check(
		"a node copied between boards without re-promotion still reports variantAnomaly",
		copied.body?.nodes?.changed?.[0]?.changes?.variantAnomaly?.to === "current",
	);
	check(
		"  and still warns about it",
	...
	check(
		"promoting on a variant board takes no --variant",
		promotedThere.code === 0,
		promotedThere.stderr.trim(),
	);
	check(
		"  and stamps the variant of the board it was promoted on",
	...
	check(
		"  and the same call on a current board still stamps current",
	...
	check(
		"  so a node promoted on each board reports no variantAnomaly",
		promotedDiff.body?.summary?.nodesChanged === 0 &&
			promotedDiff.body?.summary?.nodesUnchanged === 1,
		JSON.stringify(promotedDiff.body?.nodes?.changed?.map((c) => c.changes) ?? []),
	);
	check(
		"  and no stale-variant warning either",
	...
	check(
		"  while --variant still overrides the board it is called on",
	...
	await sleep(150);
	check(
		"branching leaves the pane holding the source exactly where it was",
		one.board() === "ledger",
		one.board(),
	);
	check(
		"  and sends it nothing, so the scene in front of the human is untouched",
	...
	check(
		"  and says what it did, naming the pane it kept and moving none",
	...
	check(
		"  and the branch is a real board, just not one on screen",
		offScreen.body?.count === 7,
		`count ${offScreen.body?.count}`,
	);
	check(
		"  carrying the unpromoted element too, content and all",
		(offScreen.body?.elements ?? []).some(
			(el) => el.type === "text" && el.text === "a note to self",
		),
	);
	...
	check(
		"  and reports the screen it left alone, pane by pane",
	...
	await api("POST", "/api/elements?board=scratch", {
		type: "rectangle",
		x: 0,
		y: 0,
		width: 40,
	...
	const namedScratch = await api("POST", "/api/boards/save?board=scratch", {
		name: "sketchbook",
		level: "module",
	});
	await sleep(150);
	check(
		"naming the scratch board takes its pane with it",
		namedScratch.body?.saveKind === "named" && two.board() === "sketchbook",
		two.board(),
	);
	check(
		"  and the answer says which pane it moved",
	...
	await sleep(80);
	check(
		"a save back to a board's own note had no screen decision to report",
		inPlace.body?.saveKind === "same-board" &&
			inPlace.body?.panes?.moved?.length === 0 &&
			inPlace.body?.panes?.kept?.length === 0,
		JSON.stringify(inPlace.body?.panes),
	);
	check(
		"  and tells panes through the same write message as every element route",
		one.seen
			.slice(beforeInPlaceSave)
			.some((message) => message.type === "elements_changed" && message.board === "ledger"),
	...
	check(
		"with room for another pane, the branch answer offers pane open --board",
		branchedRoom.code === 0 && /pane open --board ledger@option-g/.test(branchedRoom.stderr),
		branchedRoom.stderr.trim(),
	);
	check(
		"  and never names the command that would take the source off screen",
		!/board open/.test(branchedRoom.stderr),
		branchedRoom.stderr.trim(),
	);
	check(
		"  while still saying the source stayed put",
		/the only pane still holds "ledger"/.test(branchedRoom.stderr),
		branchedRoom.stderr.trim(),
	);
	...
	fs.writeFileSync(
		ledgerNote,
		fs
			.readFileSync(ledgerNote, "utf-8")
			.replace('"type": "text"', '"id": "theirledger", "type": "text"'),
	);
	...
	check(
		"a held board is the one branch that takes its pane with it: first it is held",
	...
	await sleep(150);
	check(
		"  and saving it elsewhere moves the pane onto the copy that has a home",
	...
	check(
		"  rather than reporting it kept, which is what a branch of a saving board reports",
	...
	fs.writeFileSync(
		cliNote,
		fs
			.readFileSync(cliNote, "utf-8")
			.replace('"type": "rectangle"', '"type": "rectangle", "angle": 0.5'),
	);
	const cliRefused = await cli([
		"add",
		"--board",
		"cliheld",
		"--one",
	...
	check(
		"an agent write refused for the same reason still exits 5 from the CLI",
		cliRefused.code === 5,
		`exit ${cliRefused.code}`,
	);
	check(
		"  with the three outcomes, and what happens to everything drawn from here",
	...
	check(
		"  and the write after it is taken, saying where it went",
	...
	check(
		"a board named with capitals is addressed in lower case",
		madeMixed.status === 200 && madeMixed.body?.board === "casetest",
	);
	check(
		"  and keeps the casing it was named with",
		madeMixed.body?.identity?.displayName === "CaseTest",
	);
	check(
		"  and its note is named that way too",
	...
	check(
		"the same board answers to another spelling of its name",
		otherSpelling.status === 201 || otherSpelling.status === 200,
	);
	...
	check(
		"  and writes the one note, not a second one",
		fs.readdirSync(vault).filter((f) => /^casetest\.excalidraw\.md$/i.test(f)).length === 1,
	);
	check(
		"  with the human's casing in the frontmatter",
	...
	check(
		"  and the refusal says which board it means",
		/already open|already exists/.test(collides.body?.error ?? ""),
		collides.body?.error,
	);
	...
	check(
		"starting a board over a note that differs only in case is refused",
		startedOver.status === 409,
		startedOver.body?.error,
	);
	check(
		"  and the refusal names the note it would have collided with",
		/Handover\.excalidraw\.md/.test(startedOver.body?.error ?? ""),
	);

	const openedLower = await api("POST", "/api/boards/open", { board: "handover" });
	check(
		"a note authored under another casing opens by the lower-case address",
		openedLower.status === 200 &&
			path.basename(openedLower.body?.file ?? "") === "Handover.excalidraw.md",
	);
	check(
		"  and is not reported as declaring a different board",
	...
	check(
		"a board is drawn for the id check",
		drawn.status === 200 || drawn.status === 201,
	...
	check(
		"every id the server minted is short enough to be a block reference",
	...
	check(
		"  and the note holds what the board holds, expanding nothing further",
		sceneJson.elements.length === 7,
		`${sceneJson.elements.length} elements in the note`,
	);
	...
	check(
		"  every element the board holds is in the note under the name it had",
		carried.length === stored.length,
		stored
			.map((el) => el.id)
			.filter((id) => !inNote.includes(id))
			.join(", "),
	);
	for (const el of sceneJson.elements) {
		if (!el.containerId) continue;
		check(
			`  the label bound to ${el.containerId} has a block reference`,
			idNote.includes(`^${el.id}`) && inNote.includes(el.containerId),
			el.id,
		);
		...
		check(
			`  and is named after it, so the browser would agree`,
			el.id === labelTextIdFor(el.containerId),
			`${el.id} vs ${labelTextIdFor(el.containerId)}`,
		);
	}
	...
	// against a real canvas.
	{
		...
		check(
			"a board with no note yet has nobody else's writing on it",
			noteWrittenElsewhere(watched) === null,
		);
		writeBoardContent(watchedBoard, emptyContent(), { saveCommand: "board save" });
		check(
			"  and neither has one archboard has just written itself",
			noteWrittenElsewhere(watched) === null,
		);
		...
		check(
			"a note whose size and time have not moved is not read again",
			restored.size === original.length &&
				restored.mtimeMs === pinned.getTime() &&
				noteWrittenElsewhere(watched) === null,
		...
		check(
			"a note written by something that is not archboard is seen with no write and no command",
			written?.board === watched && written?.reason === "changed",
			JSON.stringify(written),
		);
		check(
			"  saying when it was written and when archboard last saw it, not only that they differ",
		...
		check(
			"  and offering the reload alone, because nothing has been refused and nothing is held",
		...
		releaseHold(watched);
		check(
			"  and it is back the moment the hold ends without the note being taken",
		...
		recordBaseline(
			watchedBoard,
			watchedBoard.file,
			hashBoardBytes(fs.readFileSync(watchedBoard.file)),
			versionNumber(fs.readFileSync(watchedBoard.file, "utf-8")),
		);
		check(
			"taking the note clears it, with no write, no restart and no timer",
			noteWrittenElsewhere(watched) === null,
		);
	...
	try {
		...
		check(
			"  and is the one board marked a placeholder, because nobody named it",
			firstInfo.body?.placeholder === true,
			JSON.stringify(firstInfo.body?.placeholder),
		);
		...
		check(
			"saving scratch writes its own note rather than demanding a name",
			savedScratch.status === 200 && savedScratch.body?.file === scratchNote,
			savedScratch.body?.error ?? savedScratch.body?.file,
		);
		check("  and the note is on disk, in the vault's hidden directory", fs.existsSync(scratchNote));
		...
		check(
			"  without turning up in the vault's list of boards",
			(listed.body?.boards ?? []).length === 0,
			JSON.stringify(listed.body?.boards),
		);
		check(
			"  while still being open, and addressable, like any other board",
		...
		check(
			"  read back from the note, not invented empty",
		...
		check(
			"a board drawn on and never saved survives the canvas being killed",
		...
		check(
			"  with everything else on it, not just the last thing drawn",
		...
		check(
			"  and so does one that `board new` started and nobody saved",
		...
		check(
			"  holding what was drawn on it, label and all",
		...
		check(
			"a board edited on disk reads back changed, with no restart and no reload",
		...
		check(
			"  and the next write is refused, because the note changed underneath",
			refused.status === 409 && /Refusing to save/.test(refused.body?.error ?? ""),
			`${refused.status} ${String(refused.body?.error ?? "").slice(0, 60)}`,
		);
		check(
			"  offering the three outcomes rather than picking one",
			Boolean(
				refused.body?.conflict?.outcomes?.reload &&
		...
		check(
			"  and nothing was written, so the disk still holds their version",
			fs.readFileSync(notePath, "utf-8").includes('"width": 321'),
		);

		// Outcome one, and the way back: take the note.
		const reloaded = await scratchApi("POST", "/api/boards/open", {
			board: "unsaved",
			reload: true,
		});
		check(
			"  taking the note un-sticks it, which is ADR 0006's first outcome",
			reloaded.status === 200,
			reloaded.body?.error,
		);
		...
		check(
			"a write refused by the hash check leaves the board not saving",
		...
		check(
			"  and says so with the three outcomes, since when, and how much is held",
		...
		check(
			"  and the refused write is a refusal: it is not on the board",
			!(await scratchApi("GET", "/api/elements?board=holdover")).body?.elements?.some(
				(el) => el.id === "lost1",
			),
		);
		...
		check(
			"the next change is taken rather than refused again",
			held1.status === 200,
			`${held1.status} ${held1.body?.error}`,
		);
		check(
			"  and goes into the held copy, not into their note",
			!fs.readFileSync(overwriteCase.file, "utf-8").includes("held1"),
		);
		check(
			"  which is what the board now reads as",
			(await scratchApi("GET", "/api/elements?board=holdover")).body?.elements?.some(
				(el) => el.id === "held1",
			),
		);
		check(
			"  and every answer about it says it is not being saved, and counts what is held",
		...
		check(
			"  including the listing, so an agent finds out without writing to it first",
			(await scratchApi("GET", "/api/boards")).body?.open?.find((b) => b.key === "holdover")?.held
				?.board === "holdover",
		);
		...
		const savedAtWhenHeld = (await scratchApi("GET", "/api/boards/info?board=holdover")).body
			?.savedAt;
		...
		check(
			"  and the last-saved time does not move, because nothing was saved",
			(await scratchApi("GET", "/api/boards/info?board=holdover")).body?.savedAt ===
				savedAtWhenHeld,
			savedAtWhenHeld,
		);
		...
		check(
			"a pane saying what is on its screen is taken on a held board",
			fullReport.status === 200,
			`${fullReport.status} ${fullReport.body?.error}`,
		);
		const afterFullReport =
			(await scratchApi("GET", "/api/elements?board=holdover")).body?.elements ?? [];
		check(
			"  and the held copy becomes that screen, not their note plus one pending user edit",
			afterFullReport.some((el) => el.id === "held1") &&
				!afterFullReport.some((el) => el.id === "theirs1"),
			JSON.stringify(afterFullReport.map((el) => el.id)),
		);
		check(
			"  and the board says a pane has spoken for it",
		...
		check(
			"  so the next change reaches the note like any other",
		...
		check(
			"  and a pane can no longer declare the whole board, because it is saving again",
		...
		const took = await scratchApi("POST", "/api/boards/open", {
			board: "holdreload",
			reload: true,
		});
		check(
			"reload takes the note and the board saves again",
			took.status === 200 && took.body?.held === undefined,
			`${took.status} ${took.body?.error}`,
		);
		const afterReload =
			(await scratchApi("GET", "/api/elements?board=holdreload")).body?.elements ?? [];
		check(
			"  and what was held is gone, which is what reload costs",
		...
		const elsewhere = await scratchApi("POST", "/api/boards/save", {
			board: "holdelse",
			name: "holdmine",
		});
		check(
			"save elsewhere writes the held copy to a note of its own",
			elsewhere.status === 200 && fs.readFileSync(elsewhere.body?.file, "utf-8").includes("held3"),
			`${elsewhere.status} ${elsewhere.body?.error}`,
		);
		check(
			"  and says the hold is over and which outcome ended it",
			elsewhere.body?.resolvedHold?.outcome === "elsewhere" && elsewhere.body?.held === undefined,
			JSON.stringify(elsewhere.body?.resolvedHold),
		);
		check(
			"  and their note is untouched, which is what makes this the free one",
		...
		check(
			"  and the board that was held reads as their version now, saving again",
		...
		try {
			...
			await sleep(200);
			check(
				"a pane is told where the note stands as it arrives on a board",
			...
			await sleep(2400);
			check(
				"  and hears about a note written underneath it without anybody writing to the board",
			...
			check(
				"  which is not a hold and not a lock: nothing was refused and nobody is excluded",
				!heard.some((m) => m.type === "board_hold") &&
					lastNote()?.outcomes === undefined &&
					lastNote()?.writes === undefined &&
					locks[locks.length - 1]?.held === false,
				JSON.stringify({
					holds: heard.filter((m) => m.type === "board_hold").length,
					lock: locks[locks.length - 1]?.held,
				}),
			);
			...
			await sleep(200);
			check(
				"  and taking the note takes the mark down by itself",
				lastNote() === null,
				JSON.stringify(lastNote()),
			);
		} finally {
	...
	// read brought the element back renamed under whoever was drawing.
	{
		...
		check(
			"a text element whose id cannot be a block reference is renamed",
			made.status === 200 && answered !== long,
			`${made.status} ${answered}`,
		);
		check(
			"  to something a block reference can hold",
			/^[A-Za-z0-9-]{1,8}$/.test(answered ?? ""),
			String(answered),
		);
		check(
			"  and the write answers with the name the board actually holds",
			Boolean(answered) &&
		...
		check(
			"  and reading the board back finds that same name, not another one",
		...
		check(
			"  which is also the block reference in the note",
		...
		check(
			"  while an id that can already be one is left exactly as it is",
			short.body?.element?.id === "cap2",
			String(short.body?.element?.id),
		);
	}
	...
	try {
		...
		check(
			"a board is saved for the atomicity check",
		...
		check(
			"  and the vault listing sees one board named atomic, not a second",
		...
		check(
			...
			tempName.startsWith(".") && tempName.endsWith(".tmp"),
			tempName,
		);
		...
		check(
			"the bytes are flushed to disk before the rename",
			order[0] === "fsync" && order[1] === "rename",
			order.join(" -> "),
		);
		check(
	...
	const imageOn = (board, fileId, x) =>
		api("POST", `/api/elements?board=${board}`, {
			type: "image",
			x,
			y: 0,
			width: 80,
	...
	check(
		"an image is added to the board that is drawing it",
	...
	check(
		"  and one posted before anything draws it is not kept, and says so",
	...
	check(
		"  and asking one board for its images gets that board's",
		Object.keys(onlyA.body?.files ?? {}).join(",") === "img-a",
	);
	const noBoard = await api("GET", "/api/files");
	check(
		"  while asking without naming a board is refused, like every other route",
		noBoard.status === 400,
		JSON.stringify(noBoard.body?.error ?? "").slice(0, 80),
	);
	...
	check(
		"an image no element draws is left out of the note",
	...
	check(
		"a board opened from a note nothing here has held loads",
		cold.status === 200,
		cold.body?.error,
	);
	const coldFiles = await api("GET", "/api/files?board=picsc");
	check(
		"  and its images come off the disk with it",
	...
	check(
		"  so writing back what was just read keeps the image rather than dropping it",
	...
	check(
		...
		JSON.stringify(Object.keys(switched?.files ?? {})),
	);
	...
	// is on the only path that builds a scene.
	{
		...
		check(
			"a scene built from one board's elements carries only the images they draw",
		...
		check(
			"  and a board that draws none has no files key at all",
	...
	// following it without preserving it loses the record on the first save.
	{
		...
		check(
			"a note the plugin has migrated the images out of opens",
			migrated.status === 200,
			migrated.body?.error,
		);
		const migratedFiles = await api("GET", "/api/files?board=picsd");
		check(
			"  and the image it moved into the vault is followed to its file",
		...
		check(
			"  and the save keeps the section saying where it went",
			migratedNote.includes("## Embedded Files") &&
				migratedNote.includes("emb12345: [[attachments/logo.png]]"),
			savedMigrated.body?.error,
		);
		check(
			"  without writing the bytes back into a note the plugin migrated them out of",
		...
		check(
			"a bare filename is resolved against the vault",
		...
		check(
			"  but a name two files answer to is left unresolved rather than guessed at",
		...
		check(
			"a wikilink pointing outside the vault resolves to nothing",
		...
		// got wrong on exactly one path.
		{
			...
			check(
				"  and both refuse a file that is not an Obsidian note, saying the same thing",
			...
			check(
				"  and a note that is not there is null on both rather than a throw",
