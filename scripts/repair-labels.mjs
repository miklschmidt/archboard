...
const withBoard = (path) =>
	`${baseUrl}${path}${boardKey ? `${path.includes("?") ? "&" : "?"}board=${encodeURIComponent(boardKey)}` : ""}`;
...
const put = (id, patch) =>
	json(withBoard(`/api/elements/${encodeURIComponent(id)}`), {
		method: "PUT",
...
if (anchors.size > 0) {
	console.log(`Rerouted every arrow anchored to ${anchors.size} shape(s).`);
}
...
if (resized > 0)
	console.log(`Re-measured ${resized} arrow(s) whose width/height no longer matched their points.`);
...
if (moves.length > 0) {
	console.log(
		"The note on disk is unchanged until you save: `./bin/canvas board save --board <name>`.",
	);
}
