...
const postPane = async (build, clientId) => {
  const response = await fetch(`${base}/api/panes`, {
    ...
    body: JSON.stringify({
      clientId, paneId: clientId, board: 'scratch', primary: true, focused: true,
      elementCount: 0,
...
const cli = args => new Promise(resolve => {
  ...
  child.on('exit', code => {
    let json = null;
    try { json = JSON.parse(stdout); } catch { /* not JSON, which is a finding on its own */ }
...
try {
  ...
  check('a canvas that has just started is not running old source',
    first.source?.stale === false, `newest is ${first.source?.newestFile}`);
  check('  and it names the file it read last, so the comparison can be checked',
    typeof first.source?.newestFile === 'string' && first.source.newestFile.startsWith('src/'),
    String(first.source?.newestFile));
  check('  and it says when it read its source',
    !Number.isNaN(Date.parse(first.source?.evaluatedAt ?? '')), String(first.source?.evaluatedAt));

  const quiet = await cli(['status']);
  check('status says nothing about staleness while there is nothing to say',
  ...
  check('a source file written after the canvas started makes it stale',
    stale.source.stale === true);
  check('  and the canvas names that file, not just the fact',
    stale.source.newestFile === 'src/core/compare.ts', String(stale.source.newestFile));
  check('  and the process is the one that started, so nothing restarted itself',
    stale.pid === first.pid, `${first.pid} -> ${stale.pid}`);

  const loud = await cli(['status']);
  check('status reports it in the JSON, where a script reads it',
    loud.json?.stale?.changedFile === 'src/core/compare.ts',
    JSON.stringify(loud.json?.stale ?? null).slice(0, 120));
  check('  and in a sentence on stderr, where a person reads it',
  ...
  check('the canvas knows which bundle it is serving',
    typeof current === 'string' && current.startsWith('/assets/'), String(current));
  ...
  check('a pane on a bundle the canvas no longer serves is told at once',
    old.staleFrontend?.stale === true, JSON.stringify(old.staleFrontend ?? null).slice(0, 100));
  check('  in a sentence that names both builds and what to do',
  ...
  check('  and the pane is still registered, because being old is not being wrong',
    old.registered === true);

  const fresh = await postPane(current, 'p-stale-1');
  check('a pane on the bundle that is on disk hears nothing',
    fresh.staleFrontend === undefined, JSON.stringify(fresh.staleFrontend ?? null));

  const dev = await postPane('/src/main.tsx', 'p-stale-1');
  check('a tab served by the vite dev server hears nothing, having no bundle to be old',
    dev.staleFrontend === undefined, JSON.stringify(dev.staleFrontend ?? null));

  const silent = await postPane(undefined, 'p-stale-1');
  check('a client that says nothing about its build is not guessed at',
...
console.log('staleness: the canvas says when it is running old source, and a tab is told when it is old.');
