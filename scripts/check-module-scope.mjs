...
// to remember to add it. The CLI and the MCP entry point are one-shot processes
// and are not checked.
//
...
const HARMLESS_CONSTRUCTORS = new Set([
  'RegExp', 'Date', 'URL', 'URLSearchParams', 'TextEncoder', 'TextDecoder',
  'Intl', 'Number', 'String', 'Boolean', 'Symbol', 'BigInt', 'Promise',
...
const ADD_LISTENER = new Set([
  'on', 'once', 'addListener', 'addEventListener', 'prependListener', 'prependOnceListener'
]);
...
const MUTATORS = new Set(['set', 'add', 'push', 'delete', 'clear', 'unshift', 'splice', 'pop', 'shift']);
...
function walkModuleScope(node, visit) {
  ...
  if (skip) return;
  visit(node);
  ts.forEachChild(node, child => walkModuleScope(child, visit));
}

function walkAll(node, visit) {
  visit(node);
  ts.forEachChild(node, child => walkAll(child, visit));
}
...
function guardedByPresenceTest(node, receiver, source) {
  for (let current = node.parent; current; current = current.parent) {
    if (ts.isIfStatement(current)) {
      ...
      if (condition.includes(`${receiver}.has(`) ||
...
for (const file of files) {
  ...
  walkModuleScope(source, node => {
    if (ts.isImportDeclaration(node) && node.importClause) {
      ...
      if (clause.namedBindings) {
        if (ts.isNamespaceImport(clause.namedBindings)) longLived.add(clause.namedBindings.name.text);
        else for (const element of clause.namedBindings.elements) longLived.add(element.name.text);
      }
    }
  ...
  walkModuleScope(source, node => {
    ...
    if (ts.isNewExpression(node)) {
      ...
      report(node, 'new-at-module-scope',
    ...
    if ((ts.isObjectLiteralExpression(node) || ts.isArrayLiteralExpression(node))) {
      ...
      if (name && mutatedNames.has(name)) {
        report(node, 'mutable-literal-at-module-scope',
    ...
    if (ts.isIdentifier(node.expression) && TIMERS.has(node.expression.text)) {
      report(node, 'timer-at-module-scope',
    ...
    if (ADD_LISTENER.has(method)) {
      ...
      if (!pairedWithRemoval(receiver, event, removals) && !guardedByOnceFlag(node, source)) {
        report(node, 'listener-at-module-scope',
    ...
    if (BINDERS.has(method) && node.arguments.length > 0) {
      if (!guardedByOnceFlag(node, source)) {
        report(node, 'bind-at-module-scope',
    ...
    if (MUTATORS.has(method) && /^[A-Za-z_$][\w$]*$/.test(receiver) && longLived.has(receiver)) {
      if (!guardedByPresenceTest(node, receiver, source)) {
        report(node, 'mutation-at-module-scope',
 ...
 */
function selfTest() {
  const dir = path.join(repoRoot, 'scripts', 'fixtures', 'module-scope');
  const expected = [
    ['blanks-a-kept-board.ts', 'mutation-at-module-scope',
      'TASK-057: re-running boards.set() blanked the scratch board under an open pane'],
    ['answers-every-message-twice.ts', 'listener-at-module-scope',
      'TASK-057: a connection handler added rather than replaced answered every message twice'],
    ['starts-a-second-timer.ts', 'timer-at-module-scope',
      'a timer started again on every reload, with nothing stopping the last one'],
    ['binds-the-port-again.ts', 'bind-at-module-scope',
      'a port bound again on every reload'],
    ['rewinds-a-mutable-literal.ts', 'mutable-literal-at-module-scope',
  ...
  for (const [name, rule, what] of expected) {
    const { findings } = analyze([path.join(dir, name)]);
    say(findings.some(f => f.rule === rule), `${what} [${rule}]`);
  }

  const safe = analyze([path.join(dir, 'reload-safe.ts')]);
  say(safe.findings.length === 0,
