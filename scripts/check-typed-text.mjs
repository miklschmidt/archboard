...
try {
  ...
  check('the text tool is up, so the next click draws a text element',
    tool.tool === 'text', JSON.stringify(tool));
  ...
  check('a click on empty canvas opens an editor on an element Excalidraw named',
    typeof drawnId === 'string' && born.typing === '', JSON.stringify({ editing: drawnId, typing: born.typing }));
  check('  and that name is one the note writer would have to change',
  ...
  check('  a second element is moved, so a write goes out while the editor is open',
    nudged.ok === true, JSON.stringify(nudged));
  ...
  check('  the pane still holds it under the name Excalidraw gave it',
    midEdit.elements.some(element => element.id === drawnId),
    midEdit.elements.filter(e => e.type === 'text').map(e => e.id).join(', ') || 'no text elements');
  check('  and the server has never been told about it, which is why nothing renamed it',
    boardMid.get(drawnId) === undefined && !midEdit.elements.some(e => e.type === 'text' && e.id !== drawnId),
  ...
  check('  under a name the note writer keeps',
    isBlockId(drawnText?.id), String(drawnText?.id));
  check('  and the pane reads the same element the same way',
  ...
  check('a double-click on a shape opens an editor on a label Excalidraw named',
  ...
  check('  the container reached the server while the editor was open, so a write really happened',
    (boardLabelMid.get('auth')?.boundElements ?? []).some(bound => bound.id === labelId),
    JSON.stringify(boardLabelMid.get('auth')?.boundElements));
  check('  the editor is still open on the label, holding what was typed',
    labelMid.editing === labelId && labelMid.typing === 'ABCDE',
    JSON.stringify({ editing: labelMid.editing, typing: labelMid.typing }));
  check('  and the label itself is not on the server, so nothing has renamed it',
  ...
  check('  under a name the note writer keeps',
    isBlockId(label?.id), String(label?.id));
  check('  with the container naming it, so the rename took the binding with it',
    (boardLabel.get('auth')?.boundElements ?? []).some(bound => bound.id === label?.id),
    JSON.stringify(boardLabel.get('auth')?.boundElements));
  check('  and the pane agrees about both',
  ...
  const renameable = (wire.upserts ?? [])
    .filter(element => element.type === 'text' && !isBlockId(element.id));
  check('no text id this pane ever posted is one the note writer would rename',
    renameable.length === 0,
    renameable.length === 0
  ...
  // to the server.
  const names = new Set((wire.upserts ?? [])
    .filter(element => element.type === 'text').map(element => element.id));
  check('  and it saw both of them go out, which is what makes that mean something',
