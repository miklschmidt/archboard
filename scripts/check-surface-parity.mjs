...
const CLI_ONLY = {
  'repo list': 'the checkout registry is host state. It maps a repository identity to a directory on THIS machine (ADR 0010). MCP consumes it: `promote_selection` takes a repo identity and resolves through the registry. A client with no shell cannot see the filesystem those entries name, so it cannot maintain them.',
