...
try {
	...
	check(
		"a write that says nothing about what it is doing is refused",
		silent.status === 400,
		`${silent.status}`,
	);
	check(
		"  with a code a caller can act on",
		silent.body?.code === "DOING_REQUIRED",
		String(silent.body?.code),
	);
	check(
		"  naming both public surfaces, because the caller could be on either",
		/--doing/.test(silent.body?.error ?? "") && /\?doing=/.test(silent.body?.error ?? ""),
	);
	check(
		"  and saying a claim's overall reason does not satisfy the per-write requirement",
	...
	check(
		"and neither is a paragraph — the activity feed accepts bounded descriptions, not log entries",
		long.status === 400 && /140/.test(long.body?.error ?? ""),
		`${long.status}`,
	);
	...
	// The CLI says it in its own words and is refused in the same place: the
	// canvas. It keeps no second list of which commands write.
	...
	const landed = await api(
		"POST",
		`/api/elements?board=payments&${said("adding the payment queue")}`,
		box("a"),
	);
	check(
		"the same write lands once it says what it is doing",
		landed.status === 201 || landed.status === 200,
		`${landed.status}`,
	);
	...
	const saved = await api(
		"POST",
		`/api/boards/save?board=payments&${said("writing the board down")}`,
		{},
	);
	...
	check(
		"and what an agent said is nowhere in the note it wrote",
		!bytes.includes("adding the payment queue") && !bytes.includes("writing the board down"),
	);
	check(
		"  nor is the field name, so nothing carried it in under another spelling",
		!/doing/.test(bytes),
	);
	check("  while the element it was said about is in there", /"type": ?"rectangle"/.test(bytes));

	const element = (await api("GET", "/api/elements?board=payments")).body?.elements?.[0] ?? {};
	check(
		"and the board returns the element with no trace of it either",
	...
	const registerPane = (clientId, x, primary) =>
		api("POST", "/api/panes", {
			clientId,
			paneId: clientId,
			board: "scratch",
	...
	await api(
		"POST",
		`/api/elements?board=payments&${said("rerouting orders through it")}`,
		box("e", 200),
	);
	await sleep(250);
	const news = heard.left.at(-1);
	check(
		"the pane holding the board is told what was just done to it",
		heard.left.length === 1,
		`${heard.left.length} message(s)`,
	);
	check(
		"  in the writer's own words",
		news?.doing?.doing === "rerouting orders through it",
	...
	check(
		"  and it carries the recent list, not only the new line",
	...
	check(
		"the pane holding another board is sent it named as somebody else's board",
	...
	await api(
		"POST",
		`/api/elements/xxx-not-here?board=payments&${said("updating a box that is gone")}`,
		{ x: 1 },
	);
	await sleep(200);
	check(
		"a write that was refused emits no activity description",
	...
	check(
		"a person's own change report needs no line and is not refused",
		human.status === 200,
		`${human.status}`,
	);
	await sleep(250);
	check(
		"  and nothing invents one on their behalf",
		heard.left.length === 0,
		JSON.stringify(heard.left.map((m) => m.doing?.doing)),
	);
	...
	check(
		"and so is Save in the shell, which says who pressed it in the query",
		shellSave.status === 200,
		`${shellSave.status}`,
	);
	...
	for (let i = 0; i < 7; i += 1) {
		await api(
			"POST",
			`/api/elements?board=payments&${said(`step ${i}`)}`,
			box(`s${i}`, 400 + i * 20),
		);
	}
	await sleep(250);
	...
	check(
		"the list is the last few actions, not a transcript",
		list.length === 5,
		`${list.length} kept`,
	);
	check(
	...
	for (let i = 0; i < 3; i += 1) {
		await api(
			"POST",
			`/api/elements?board=payments&${said("restoring the payment path from the export")}`,
			box(`r${i}`, 600 + i * 20),
		);
	}
	await sleep(250);
	...
	check(
		"one thing said three times running is one line, not three",
		repeated.filter((entry) => entry.doing === "restoring the payment path from the export")
			.length === 1,
		repeated.map((entry) => entry.doing).join(" | "),
	);
	check(
		"  and it is the newest line, so the list still reads in order",
	...
	await sleep(300);
	check(
		"a pane opened partway through ongoing agent work receives the recent activity",
