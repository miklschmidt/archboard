...

{
	...
	assert(
		indices.every((value, i) => i === 0 || indices[i - 1] < value),
...

{
	...
	const bare = expandElements(
		[{ id: "bare", type: "rectangle", x: 0, y: 0, width: 10, height: 10 }],
		{ forStore: true },
	);
...

{
	...
	assert(
		seedOf(fresh.find((el) => el.id === "svc")) === undefined,
...

{
	...
	const written = expandForBoard(
		[{ ...board.get("svc"), label: { text: "IdentityService" } }],
		board,
	);
...

{
	...
	assert(labelAnchorOf(stale)?.x === 100, "an arrow trusted its stale width instead of its points");
...

{
	...
	assert(
		strayed[0].distance > 800,
		`the stranded label reads ${Math.round(strayed[0].distance)}px from its box`,
	);
...

{
	...
	assert(
		Math.round(rescue[0].x) === 275 && Math.round(rescue[0].y) === 27,
...

{
	...
	try {
		...
		await api("PUT", `/api/elements/wire${board}`, {
			points: [
				[0, 0],
				[400, 500],
			],
		...
		assert(
			seedOf(renamed.find((element) => element.id === "svc")) === undefined,
		...
		assert(
			saved?.success === true,
			`saving the board failed: ${JSON.stringify(saved?.error ?? saved)}`,
		);
