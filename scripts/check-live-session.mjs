...
if (which.error) {
	console.error(
		"live-session: agent-browser is not on PATH, so there is no pane to disagree with us.",
	);
	console.error(
		"  A socket cannot stand in here: the point is what Excalidraw does to a document.",
	);
	process.exit(2);
}
...
const humanEdit = (edit) => evalInPage(`window.__abApplyEdit(${JSON.stringify(edit)})`);
...
const measurementNoise = (element, key, a, b) => {
	...
	return (
		Number.isFinite(ours) && Number.isFinite(theirs) && Math.abs(ours - theirs) < MEASURER_EPSILON
	);
};
...
try {
	...
	check(
		"a board is seeded with labelled shapes, a bound arrow and a standalone text",
	...
	check(
		"  and saved, so the session starts from the note the exporter writes",
	...
	check(
		"the pane and the server agree before anybody writes",
		start.agreed,
		(start.divergences ?? []).slice(0, 4).join(" | "),
	);
	...
	check(
		"the first A hold is delayed before the pane switches away",
	...
	for (let attempt = 0; attempt < 40; attempt++) {
		...
		returnedToA =
			listed?.board === BOARD && scene.elements?.some((element) => element.id === "auth");
	...
	check(
		"returning to A starts a distinct second delayed hold",
		delayedHolds?.pending?.length === 2 && delayedHolds.pending.every((board) => board === BOARD),
		JSON.stringify(delayedHolds),
	);
	...
	check(
		"A1 finishing cannot clear A2 or schedule an old retry",
	...
	check(
		"the second A hold remains owned and its edit persists",
		releasedA2?.board === BOARD &&
			holdRaceAgreement.agreed &&
		...
		JSON.stringify({
			releasedA2,
			before: beforeSecondHold?.x,
			after: afterHoldRace?.x,
	...
	for (let cycle = 1; cycle <= CYCLES; cycle++) {
		...
		const wrote = await api("POST", `/api/elements/changes?board=${BOARD}`, {
			origin: "agent",
			upserts,
		});
		if (wrote.status !== 200) {
			check(
				`cycle ${cycle}: the agent's ${agentMove} was accepted`,
				false,
	...
	check(
		`${CYCLES} cycles of interleaved agent and human writes, and the two documents ` +
	...
	check(
		"  and the session really did write both sides of the loop",
	...
	check(
		"  and applying a server update never started another change report",
		bounced === 0,
		`${bounced} cycles reported more than the user's own edit`,
	);
	...
	const duringServerUpdate = async (label, agentUpserts, edit, reads, wants) => {
      ...
      return { armed: true };
		...
		await api("POST", `/api/elements/changes?board=${BOARD}`, {
			origin: "agent",
			upserts: agentUpserts,
		});

		let fired = null;
		for (let i = 0; i < 100; i++) {
			fired = await evalInPage(
				"(() => ({ injected: window.__abInjected, armed: !!window.__abPending }))()",
			);
			if (!fired.armed) break;
			await sleep(50);
		}
		check(
			`${label}: the user edit lands between the server update and its record`,
			fired && !fired.armed,
		...
		const settled = await agree();
		check(
			`  and the two documents agree afterwards`,
			settled.agreed,
			(settled.divergences ?? []).slice(0, 4).join(" | "),
		);

		const after = (await held()).find((e) => e.id === edit.id);
		const got = reads(after);
		check(
			`  and the server holds the user edit, not the earlier server update`,
	...
	check(
		"  and the board still carries a label to be retyped into",
		typeof storeLabel?.id === "string",
		JSON.stringify(storeLabel?.id),
	);
	await duringServerUpdate(
	...
	check(
		"the sparse-drag acknowledgement stays in flight through another progress deadline",
	...
	check(
		"  and the overdue progress report carrying both drags is not dropped",
		bothDrags.agreed,
		(bothDrags.divergences ?? []).slice(0, 4).join(" | "),
	);
	const dragged = (await held()).find((e) => e.id === "store");
	check(
		"  so both user moves are on the board",
	...
	check(
		"  the pane can be named, so its hold can be taken out from under it",
	...
	check(
		"  and it was holding the board, taken at the first change of the edit",
		given.body?.released === true,
		JSON.stringify(given.body),
	);
	...
	check(
		"a broadcast arriving mid-drag leaves the unreported drag in the scene",
	...
	check(
		"  and the comparison names that difference, so the agreements above are real",
	...
	check(
		"  and the drag reaches the server once the report goes out",
		afterBoth.agreed,
		(afterBoth.divergences ?? []).slice(0, 4).join(" | "),
	);
	const finalBoard = await held();
	const landed = finalBoard.find((e) => e.id === "auth");
	const recoloured = finalBoard.find((e) => e.id === "queue");
	check(
		"  with both writers' work on the board",
		landed &&
			Math.abs(landed.x - (victim.x + 40)) < 0.001 &&
			recoloured?.backgroundColor === "#ff8787",
	...
	check(
		"a board that is saving says it is in the vault, and claims nothing is unsaved",
	...
	check(
		"  a note written underneath is on screen before anybody writes to the board",
		/note changed on disk/.test(noticed.elsewhere ?? ""),
		noticed.elsewhere,
	);
	check(
		"  with no dialog, because nothing has been refused and nothing is held",
		noticed.dialog === null &&
	...
	check(
		"a note rewritten underneath stops the board saving, mid-session",
	...
	check(
		"  and nothing opened in front of the human, who was drawing",
		chrome.dialog === null,
		chrome.dialog,
	);
	check(
		"  while the bar says the board is not being saved, and how much is held",
		/not saving/.test(chrome.mark ?? ""),
		chrome.mark,
	);
	...
	check(
		"  and the pane and the server still hold the same document",
		heldAgreed.agreed,
		(heldAgreed.divergences ?? []).slice(0, 4).join(" | "),
	);
	check(
		"  which is this pane's board, not the one the other editor wrote",
	...
	check(
		"  and clicking the mark is what offers the three outcomes",
		/not being saved/.test(offered.title ?? "") && offered.choices.length === 3,
		`${offered.title} ${JSON.stringify(offered.choices)}`,
	);
	check(
		"  each one of them, in the order that puts the free one nearest",
	...
	check(
		"  and overwriting writes the held board over their note",
		!noteAfter.includes('"theirs"') && noteAfter.includes('"queue"'),
		noteAfter.includes('"theirs"')
			? "their element is still in the note"
			: noteAfter.includes('"queue"')
				? ""
				: "queue is missing from the note",
	);
	const backToNormal = await evalInPage(
		`(() => ({ mark: document.querySelector('.chip-held')?.textContent ?? null }))()`,
	);
	check(
		"  and the mark comes down, because the board is saving again",
		backToNormal.mark === null,
		backToNormal.mark,
	);
	...
	check(
		"a pane on a board nobody is writing accepts edits",
		free.view === false,
		JSON.stringify(free),
	);
	...
	check(
		"  and stays locally editable while somebody else has the mutex",
		taken.view === false,
	...
	check(
		"a human edit remains visible after its first hold attempt loses",
		delayedEdit.ok === true &&
			localDelayed &&
			beforeDelayed &&
	...
	check(
		"  while persistence still waits for the authoritative mutex",
	...
	check(
		"  and a later single-flight hold retry persists the still-visible edit",
		delayedConvergence.agreed,
		(delayedConvergence.divergences ?? []).slice(0, 4).join(" | "),
	);
	await sleep(LOCK_FREE_LINGER_MS + 200);
	const back = await viewMode();
	check(
		"  and remains editable when the other writer is done",
		back.view === false,
		JSON.stringify(back),
	);
	...
	check(
		"a pane whose board an agent claimed says who has it and why",
		typeof claimed.what === "string" && claimed.what.includes(claimWhy),
		JSON.stringify(claimed),
	);
	check(
		"  while keeping the local canvas editable",
		claimed.view === false,
		JSON.stringify(claimed),
	);
	check(
		"  and offers the person the one thing they may always do",
		claimed.take === "Take back control",
		JSON.stringify(claimed),
	);
	...
	check(
		"panning and zooming a claimed board sends no hold or content report",
	...
	const claimedWrite = await api(
		"POST",
		`/api/elements?board=${BOARD}&doing=${encodeURIComponent(step)}`,
		{
			type: "rectangle",
			x: 820,
			y: 60,
	...
	check(
		"  and the agent can still write under the claim after those camera changes",
		claimedWrite.status === 200,
		`${claimedWrite.status} ${claimedWrite.body?.error ?? ""}`,
	);
	const narrated = await bannerWhen((seen) => seen.steps.some((line) => line.includes(step)));
	check(
		"  and the pane shows what the agent is doing right now, not only what it claimed the board for",
		narrated.steps.some((line) => line.includes(step)),
		JSON.stringify(narrated.steps),
	);
	check(
		"  which reads as one explanation with the banner rather than two accounts of it",
		typeof narrated.what === "string" &&
			narrated.what.includes(claimWhy) &&
			narrated.steps.some((line) => line.includes(step)),
		`${narrated.what} / ${narrated.steps.join(" | ")}`,
	);
	check(
		"  and the bar carries the latest line too, for a pane the user is not viewing",
		typeof narrated.bar === "string" && narrated.bar.includes(step),
		String(narrated.bar),
	);
	...
	check(
		"  and the claimed element can be framed without taking the board",
		framedTakeover.status === 200,
		`${framedTakeover.status} ${framedTakeover.body?.error ?? ""}`,
	);
	await sleep(700);
	...
	const queueLocalTakeover = (await readScene()).elements.find(
	...
	check(
		"trusted pointer input remains locally responsive while a claim orders persistence",
		queueBeforeTakeover && queueLocalTakeover && queueLocalTakeover.x > queueBeforeTakeover.x + 20,
		JSON.stringify({ dragPoint, before: queueBeforeTakeover?.x, local: queueLocalTakeover?.x }),
	);
	check(
		"  and the local drag is visible before its progress report persists",
		queueServerBeforeReport &&
			queueBeforeTakeover &&
	...
	check(
		"  with one single-flight hold request for the gesture",
	...
	check(
		"  and the persisted board converges on that local drag",
		takeoverAgreed.agreed,
		(takeoverAgreed.divergences ?? []).slice(0, 4).join(" | "),
	);
	...
	const explicitClaim = await bannerWhen(
		(seen) => typeof seen.what === "string" && seen.what.includes(explicitWhy),
	);
	check(
		"the explicit take-back control remains available after content takeover was added",
		explicitClaim.take === "Take back control",
		JSON.stringify(explicitClaim),
	);
	...
	check(
		"  and the activation reaches the button",
	...
	check(
		"and one activation takes the board back",
		returned.what === null && returned.view === false,
	...
	check(
		"a pane that has lost the socket assumes the board is held, not free",
		orphaned.view === true,
