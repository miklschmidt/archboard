...
// canvas through it, on the same path `library insert` takes.
...
const elementsOn = async (board) =>
	(await api("GET", `/api/elements?board=${encodeURIComponent(board)}`)).body?.elements ?? [];
...
try {
	...
	const nodesOn = async (board) =>
		new Set((await elementsOn(board)).map((el) => el.customData?.archboard?.node).filter(Boolean));
	check(
		"  with its three nodes promoted",
		(await nodesOn("payments")).size === 3,
		[...(await nodesOn("payments"))].join(", "),
	);

	// --- the proposal, branched from it ------------------------------------

	const branched = await api("POST", "/api/boards/save?board=payments", {
		name: "payments",
		variant: "option-a",
	});
	check(
		"a proposal starts as a branch of the board it proposes against",
	...
	const branchDiff = (await api("GET", "/api/boards/compare?from=payments&to=payments@option-a"))
		.body;
	const branchWrong = notABranch(branchDiff);
	check(
		"a branched proposal compares as the one thing that changed",
		branchWrong.length === 0,
		branchWrong.join("; "),
	);
	check("  three nodes are the same three nodes", branchDiff?.summary?.sharedNodes === 3);
	check(
		"  the added node is named, and it is the only one",
	...
	check(
		"  the two arrows nobody touched read as unchanged",
		branchDiff?.summary?.edgesUnchanged === 2,
	);
	check(
		"  and the new arrow is the one edge added",
	...
	check(
		"  and no shared node reads as changed",
		branchDiff?.summary?.nodesChanged === 0,
	...
	const redrawDiff = (await api("GET", "/api/boards/compare?from=payments&to=payments@redraw"))
		.body;
	const redrawWrong = notABranch(redrawDiff);
	check(
		"a redraw of the same architecture is not a branch",
		redrawWrong.length > 0,
		redrawWrong.join("; "),
	);
	check(
		"  two words changed cost two nodes",
	...
	check(
		"  and the rewordings arrive as new nodes beside the real addition",
		redrawDiff?.summary?.nodesAdded === 3,
		`added ${redrawDiff?.summary?.nodesAdded}`,
	);
	check(
		"  and no edge survives, because an edge is its two endpoints",
		redrawDiff?.summary?.edgesUnchanged === 0,
	);
	check(
		"  so the one real change is not findable in the diff",
	...
	await draw(
		...
		[
			["Edge Proxy", "Order Handling"],
			["Order Handling", "Order Store"],
			["Order Handling", "Order Cache"],
		],
	);
	...
	check(
		"an independently drawn proposal shares no node identity at all",
		freshDiff?.summary?.sharedNodes === 0,
	);
	check(
		"  so the branch test fails on it",
		notABranch(freshDiff).length > 0,
		notABranch(freshDiff).join("; "),
	);
	check(
		"  and compare says it could not compare them, not that they agree",
		freshDiff?.summary?.comparable === false && freshDiff?.summary?.identical === false,
	);
	check(
		"  and says what to do instead",
		(freshDiff?.warnings ?? []).some(
			(w) => /share no node ids/.test(w) && /copy of the current board/.test(w),
		),
	);
	...
	check(
		"the shipped library holds a stencil drawn out of nothing but lines",
	...
	const dbNode = await promoteTogether(
		"storage",
		stencil.elements.map((el) => el.id),
		"datastore",
		"current",
		"Ledger DB",
	);
	check(
		"  promoting the whole stencil gives it one node id",
		dbNode === "ledger-db",
		String(dbNode),
	);
	const promoted = (await elementsOn("storage")).filter(
		(el) => el.customData?.archboard?.node === "ledger-db",
	);
	...
	check(
		"  and the read-back counts it as a node",
		/\(2 nodes,/.test(readBack),
	...
	const storageBranch = await api("POST", "/api/boards/save?board=storage", {
		name: "storage",
		variant: "option-a",
	});
	check(
		"the board branches with the stencil node on it",
	...
	const storageDiff = (await api("GET", "/api/boards/compare?from=storage&to=storage@option-a"))
		.body;
	check(
		"  and compare sees the stencil node on both sides",
		storageDiff?.summary?.sharedNodes === 2,
		`sharedNodes ${storageDiff?.summary?.sharedNodes}`,
	);
	check(
		"  reports it unchanged by name",
		(storageDiff?.nodes?.unchanged ?? []).some((n) => n.node === "ledger-db"),
		JSON.stringify((storageDiff?.nodes?.unchanged ?? []).map((n) => n.node)),
	);
	check(
		"  not as something added or removed",
		storageDiff?.summary?.nodesAdded === 1 &&
			storageDiff?.summary?.nodesRemoved === 0 &&
			storageDiff?.nodes?.added?.[0]?.node === "ledger-replica",
	);
	check(
		"  and no line of it is left over as a connector bound to nothing",
	...
	check(
		"an arrow swept into a promotion joins the node it was promoted with",
		sweptIn === "worker",
		String(sweptIn),
	);
	...
	check(
		"  the read-back counts it once, as part of that node",
		/\(2 nodes, 0 edges/.test(wiringRead),
		wiringRead.split("\n").find((l) => l.startsWith("Total elements")),
	);
	...
	check(
		"  and stops being an edge, rather than being counted as both",
	...
	check(
		"  which compare says out loud, naming the connection that went",
		(wiringDiff?.warnings ?? []).some(
			(w) => /includes a connector/.test(w) && /"Web"/.test(w) && /"Worker"/.test(w),
		),
		JSON.stringify(wiringDiff?.warnings ?? []),
	);
	check(
		"  and both nodes are still the same two nodes",
	...
	check(
		"every eval says how it is graded",
		evals.length > 0 && evals.every((e) => typeof e.graded_by === "string" && e.graded_by),
		evals
	...
	check(
		"  the branching one names this check",
		evals.some((e) => e.graded_by === SELF),
	);
	check(
		"  every other grader is a human reading a transcript, or another script that exists",
		evals.every((e) => e.graded_by === "human" || fs.existsSync(path.join(repoRoot, e.graded_by))),
		evals
			.filter((e) => e.graded_by !== "human" && !fs.existsSync(path.join(repoRoot, e.graded_by)))
			.map((e) => e.graded_by)
			.join(","),
	);
} finally {
