...
// reader, but it is not a document Excalidraw can safely render.
{
	...
	let valid = true;
	try {
		validateRenderGeometry([{ id: "point", type: "rectangle", x: -10, y: 0, width: 0, height: 0 }]);
	} catch {
		valid = false;
	}
...
// Everything without a path keeps the answer it already had.
{
	...
	assert(
		extentOf({ type: "arrow", x: 5, y: 6, points: [] }).width === 0,
...
// recommends waypoints for routing round obstacles (TASK-044).
{
	...
	const straightText = expandElements([straight], { deterministic: true }).find(
		(el) => el.type === "text",
	);
	assert(
		near(straightText.x + straightText.width / 2, 100, 1) &&
...

{
	...
	try {
		...
		await api("POST", `/api/elements/batch${board}`, {
			elements: [
				...
				},
				{
					...
					customData: node("northwest", "Northwest"),
				},
		...
		// the size follows it.
		await api("PUT", `/api/elements/to-west${board}`, {
			points: [
				[0, 0],
				[-900, -400],
			],
		...
		assert(
			near(repointed.width, 900) && near(repointed.height, 400),
		...
		for (const el of scene) {
			...
			for (const [px, py] of el.points) {
				...
				if (x < minX - 1 || x > maxX + 1 || y < minY - 1 || y > maxY + 1)
					outside.push(`${el.id} (${Math.round(x)},${Math.round(y)})`);
			}
		}
		assert(
			outside.length === 0,
		...
		assert(
			near(minX, Math.min(...edges.map((e) => e.x0)), 1) &&
				near(minY, Math.min(...edges.map((e) => e.y0)), 1) &&
				near(maxX, Math.max(...edges.map((e) => e.x1)), 1) &&
				near(maxY, Math.max(...edges.map((e) => e.y1)), 1),
		...
		assert(
			withNorth.length === 1 && withNorth[0].length === 2,
		...
		assert(
			path(afterUnrelatedMove) === path(asLeft),
		...
		assert(
			path(await wire("arr")) === path(loose),
