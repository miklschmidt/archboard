...
test("the live canvas socket crosses the real gateway and approval broker exactly once", async () => {
	...
	const actions: BrowserWorkbenchActions = {
		...
		ordinaryApprovals: {
			pending: (candidate) => {
				try {
					return approvals.toBrowserApproval(candidate);
				} catch {
					return null;
				}
			},
	...
	const projection = {
		read: (): BrowserProjection => ({
			...
			approvals: approvals
				.inspect()
				.flatMap((approval) =>
					approval.state === "pending" ? [approvals.toBrowserApproval(approval.requestId)] : [],
				),
			dynamicApprovals: [],
			semantic: null,
			coordinator: {
	...
	sockets.on("connection", (socket) => {
		...
		socket.on("message", (raw) => {
			void owner.handle(instance, "browser-public", JSON.parse(raw.toString()), {
				send: (message) => socket.send(JSON.stringify(message)),
			});
		});
		socket.on("close", () => void owner.close(instance, "browser-public"));
	});
	...
	const openClient = async () => {
		...
		return {
			client,
			events,
			request: (action: string, extra: Record<string, unknown> = {}) => {
	...
	let second: Awaited<ReturnType<typeof openClient>> | null = null;
	try {
		await first.request("connect");
		await first.request("subscribe");
		const claimed = await first.request("claimLease");
		...
		approvals.receive(approvalRequest);
		...
		const firstApproval = await first.request("command", { command: approvalCommand });
		const duplicateApproval = await first.request("command", { command: approvalCommand });
