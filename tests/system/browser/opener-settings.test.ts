...
const githubHref =
  "https://github.com/acme/archboard/actions/workflows/check.yml";
...
async function requests(
  ...
  return browser.eval<RecordedRequest[]>(
    "window.__openerProbe?.requests ?? []",
  );
}

async function dialogSnapshot(
  browser: AgentBrowserSession,
): Promise<DialogSnapshot | null> {
...
test("global opener settings validate, test, reset, and save through the rendered shell", async () => {
  ...
  expect(await browser.eval<string>("navigator.userAgent")).toMatch(
    /headless/i,
  );
  await pollUntil(
    () =>
  ...
  await fill(
    browser,
    '.opener-argument input[aria-label="Argument 2"]',
    "--without-path",
  );
  ...
  for (const [index, argument] of draftSelection.argv.entries()) {
    await fill(
      browser,
      `.opener-argument input[aria-label="Argument ${index + 1}"]`,
      argument,
    );
  }
  await pollUntil(
  ...
  const reset = await pollUntil(
    () => dialogSnapshot(browser),
    (value) =>
      value?.current === "System default" &&
      value.effective === "xdg-open {path}",
