...
const inputIngressAllowlist = new Set([
	...
	"src/runtime/engine/lib/native-element-builders.ts",
]);
...
describe("vendor-derived board element policy", () => {
	...
	test("the pinned package's authoritative math declarations own projected geometry", () => {
		const config = JSON.parse(fs.readFileSync(path.join(repoRoot, "tsconfig.json"), "utf8"));
		expect(config.compilerOptions.paths["@excalidraw/math"]).toEqual([
			"./node_modules/@excalidraw/excalidraw/dist/types/math/index.d.ts",
		]);
		const source = fs.readFileSync(
			path.join(repoRoot, "node_modules/@excalidraw/excalidraw/dist/types/math/types.d.ts"),
			"utf8",
		);
