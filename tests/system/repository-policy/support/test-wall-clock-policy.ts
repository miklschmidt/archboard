...
function preloadErrors(bunfig: string): string[] {
	...
	if (typeof document !== "object" || document === null) return ["bunfig.toml has no [test] table"];
...
function declarationObjectErrors(file: string, object: AstNode, testName: string): string[] {
	...
	for (const property of properties) {
		...
		if (fields.has(key)) errors.push(`${file}: wall-clock declaration repeats field ${key}`);
		if (isNode(property.value)) fields.set(key, property.value);
	}
	for (const field of REQUIRED_FIELDS)
		if (!fields.has(field)) errors.push(`${file}: wall-clock declaration needs field ${field}`);
...
function declarationErrors(file: string, source: string): string[] {
	...
	walk(ast, (node) => {
		...
		if (!specifier || !helperImport(file, specifier)) return;
		...
		for (const imported of specifiers) {
			...
			if (importedName !== DECLARATION) continue;
			const localName = identifierName(imported.local);
			if (localName !== DECLARATION) errors.push(`${file}: ${DECLARATION} must not be aliased`);
			else canonicalImport = true;
			if (isNode(imported.imported)) allowedIdentifiers.add(imported.imported);
			if (isNode(imported.local)) allowedIdentifiers.add(imported.local);
		}
	});

	walk(ast, (node, ancestors) => {
		if (node.type !== "CallExpression" || identifierName(node.callee) !== DECLARATION) return;
		if (isNode(node.callee)) allowedIdentifiers.add(node.callee);
		if (!canonicalImport)
...
export function inspectTestWallClockPolicy(input: TestWallClockPolicyInput): string[] {
	...
	for (const [file, source] of input.sources) {
		if (!source.includes(DECLARATION) && !source.includes("test-wall-clock")) continue;
