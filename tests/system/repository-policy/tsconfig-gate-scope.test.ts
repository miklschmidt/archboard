...
test("every retained TypeScript file is a compiler root and a real lint input", () => {
	const compiler = Bun.spawnSync(
		[join(repoRoot, "node_modules/.bin/tsc"), "--showConfig", "-p", "tsconfig.json"],
		{ cwd: repoRoot, stdout: "pipe", stderr: "pipe" },
	);
	expect(compiler.exitCode, compiler.stderr.toString()).toBe(0);
	const config: { files: string[]; compilerOptions: Record<string, unknown> } = JSON.parse(
		compiler.stdout.toString(),
	);
	const roots = new Set(config.files.map((file) => realpathSync(resolve(repoRoot, file))));
