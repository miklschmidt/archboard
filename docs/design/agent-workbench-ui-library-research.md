...
3. Use [`@assistant-ui/react`](https://github.com/assistant-ui/assistant-ui/blob/main/packages/react/package.json) through `useExternalStoreRuntime` and `AssistantRuntimeProvider` for the executable workhorse, and `ReadonlyThreadProvider` for coordinator and inspect-only history. Archboard owns every renderer and copies no assistant-ui Elements. Direct imports are assigned narrowly: runtime/providers to TASK-143.03.02, message primitives to TASK-143.03.04, and composer primitives to TASK-143.03.05. Do not use `AssistantTransport`; it would create a second snapshot-streaming protocol beside app-server.
...
5. Implement mic capture, waveform/level display, permission state, and app-server realtime coordination as the deep `src/ui/codex-realtime` browser module with one framework-neutral public entrypoint. Its current consumer is Archboard; workspace packaging, publication, and compatibility promises remain out of scope. Exclude Archboard runtime, React, assistant-ui, generated protocol, and Node imports so the media lifecycle remains extraction-ready without bypassing the repository's source boundaries. Add a VAD package only if automatic speech segmentation becomes a measured need.
6. Defer diff UI until the workflow requires it. A later task may study assistant-ui's `CodeDiff` as a reference, but the accepted workbench copies no Elements. Keep Codex file-change approval at the request level rather than inventing unsupported per-hunk semantics.
...
app-owned reducer and command adapter
        |
assistant-ui useExternalStoreRuntime + providers
        |
...
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| [assistant-ui](https://github.com/assistant-ui/assistant-ui)            | MIT. `@assistant-ui/react` is a runtime package; Elements are optional copied source that Archboard will not adopt. | Headless conversation/thread primitives, external-store integration, composer cancellation, and optional UI examples. | React 18/19 and bundler-neutral. `useExternalStoreRuntime` adapts an existing external store; copied Elements would add unowned Tailwind/Radix/Lucide assumptions.                                                                                    | Adopt only the pinned runtime and assigned headless primitives.                                |
...
The package inspected was `@assistant-ui/react` 0.15.17, licensed under the repository's [MIT license](https://github.com/assistant-ui/assistant-ui/blob/main/LICENSE) with React 18/19 peer support. Its package manifest includes `@assistant-ui/core`, `@assistant-ui/store`, `@assistant-ui/tap`, `assistant-stream`, `assistant-cloud`, Radix, `react-textarea-autosize`, `safe-content-frame`, Zod, and Zustand. Cloud is not required by `useExternalStoreRuntime`, but it is still a package dependency and should be included in a bundle-size inspection before acceptance.
...
Every Archboard-owned approval or composer composition must preserve visible focus, accessible names, focus return after dialogs, logical reading order, and keyboard access.
...
| Conversation, thread, tool, approval, and cancellation composition | `@assistant-ui/react` with `useExternalStoreRuntime` and the assigned providers/primitives                                          | Runtime package, with Archboard adapter |
