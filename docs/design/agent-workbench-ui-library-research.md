...
5. Implement mic capture, waveform/level display, permission state, and app-server realtime coordination as a standalone private browser workspace package with one framework-neutral public API. Keep it unpublished under the repository policy, but exclude Archboard, React, assistant-ui, generated protocol, and Node imports so a later publication decision requires no extraction rewrite. Add a VAD package only if automatic speech segmentation becomes a measured need.
