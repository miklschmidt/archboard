...
1. Keep Codex app-server JSON-RPC as the source of truth and retain Archboard's existing Unix WebSocket client with its socket ownership checks.
2. Generate TypeScript protocol definitions from the installed Codex version, then add an app-owned adapter that reduces app-server threads, turns, items, approvals, and realtime events into UI state.
3. Use [`@assistant-ui/react`](https://github.com/assistant-ui/assistant-ui/blob/main/packages/react/package.json), specifically `ExternalStoreRuntime`, for accessible conversation primitives, tool rendering, composer actions, and thread selection. Do not use `AssistantTransport`; it would create a second snapshot-streaming protocol beside app-server.
...
5. Implement mic capture, waveform/level display, permission state, and app-server realtime coordination locally with browser APIs. Add a VAD package only if automatic speech segmentation becomes a measured need.
...
This leaves one canonical state machine and one wire protocol. The main gap is intentional: no reviewed library covers the complete Codex app-server lifecycle, especially bidirectional approvals and realtime voice, in a Vite-ready React component.
...
`src/runtime/engine/app-server-control.ts` already connects to the Codex control socket with `ws+unix`, verifies the socket and its owner, initializes the connection, correlates requests, and receives notifications. Today it deliberately supports injection and steering, and declines server-initiated requests because approvals are outside that feature's scope. A workbench would need to extend that lifecycle explicitly; merely rendering an approval card would not make the current client an approval-owning app-server client.
...
Codex app-server JSON-RPC
        |
existing Unix WebSocket client + generated current protocol types
        |
...
The reducer owns ordering, request IDs, approval expiry, interruption, reconnect recovery, and exactly-once responses. The UI library owns rendering and interaction composition. Neither should own the other's protocol.
...
| [shadcn/ui](https://ui.shadcn.com/docs/registry/getting-started)        | MIT. Registry source is copied into the app; declared registry and package dependencies are then installed.                        | General primitives and blocks, plus an installation format. It is not an agent runtime.                                                                                             | Vite is supported, but the standard component source assumes Tailwind and either Base UI or Radix.                                                                                                                                                    | Use the registry model only through selected assistant-ui elements. Do not add a general shadcn layer now. |
...
- [`ThreadList`](https://www.assistant-ui.com/elements/thread-list) for the task picker, provided Archboard remains the owner of list, rename, archive, active, and reconnect state.
...
It does not expose the full long-lived, bidirectional app-server contract: server-initiated approval requests, task listing and mutation, realtime methods, or Archboard's Unix WebSocket transport. It is therefore not a workbench client SDK.
...
- its [README](https://github.com/BrandonMJohnson/codex-client/blob/main/README.md) identifies stdio as the stable transport, while Archboard already relies on an owned Unix WebSocket endpoint and its socket security checks;
...
- adopting it would therefore remove current transport behavior while still requiring local code for the workbench's newest protocol surface.

[`@usetemi/codex-sdk`](https://github.com/usetemi/codex-sdk/blob/main/packages/typescript/README.md) 0.133.0 is MIT and adds low-level app-server helpers around the official exec SDK, with Node 24 or later and dependencies on matching older Codex packages plus the MCP SDK. It does not improve on Archboard's existing Unix WebSocket boundary and would add version and dependency coupling.
...
Recommendation: adopt none of these client SDKs. Retain `AppServerControl`'s Unix WebSocket transport, ownership checks, and initialization behavior; generate definitions from the installed Codex; and expand the client deliberately to handle server requests, approvals, workbench thread methods, interruption, and realtime. The current automatic decline of server requests must become an explicit lifecycle with a single UI owner before approvals are enabled.
...
| ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| Canonical protocol and state                                     | Existing app-server client plus generated installed-version types and an app reducer                                          | Archboard                               |
| Conversation, task, tool, approval, and cancellation composition | `@assistant-ui/react` with `ExternalStoreRuntime`                                                                             | Runtime package, with Archboard adapter |
| Visual components                                                | A minimal subset of assistant-ui ThreadList, Composer, ToolFallback/ApprovalCard, and StoppedRun, restyled to existing tokens | Copied and owned source                 |
...
Do not initially add AI SDK, AI Elements, Apps SDK UI, ChatKit, a general shadcn installation, a third-party chat registry, a voice framework, a diff runtime, or a Codex client SDK.
