...
- `tests/system/repository-policy/ci-browser-gate.test.ts`: the mixed-use static and preflight assertions remain applicable non-browser checks. Two executable-adapter cases are phase-deferred unchanged because they launch the browser owner after the frontend build: “canonical owner argv ignores substitution input and receives the configured executable” and “scopes the long agent-browser operation timeout to the human-performance owner.” During quarantine both currently stop at the intentionally absent `frontend/index.html`; this is not passing evidence. TASK-150.06 must run both unchanged after reconstruction and restore the complete normal gate. No fake entrypoint, skipped assertion or permanent owner exclusion is permitted.
- `tests/system/boards/server-rendering-failure.test.ts`: source, assertions and strict compiler/lint ownership remain active. Its focused execution is phase-deferred because the retained failure owner first requires the rebuilt `dist/frontend`; during quarantine it stops before reaching its intended missing-Chromium assertion. TASK-150.06 must run the case unchanged after frontend reconstruction. The absent distribution is not passing evidence and authorizes neither a fake entrypoint nor a normal test skip.
