...
// is gone. Text width is measured and never estimated: by the canvas of the
// place drawing the picture, with the diagram's own faces loaded
// (`lib/canvas-text.ts`).
//
...
// element, so a string set at 500 is measured in the medium face and one set
// at 400 in the regular. Passing a family alone is what made a name fitted in
// one face overflow when drawn in another, and there is no spelling for it any
// more.

import { canvasWidth } from "@/transformers/semantic-renderer/lib/canvas-text";
import type { DiagramFont } from "@/transformers/semantic-renderer/lib/fonts";

const ELLIPSIS = "…";

/**
 * Width of one line in diagram units, measured by the host's canvas.
 * @param text The line.
...
function measure(text: string, font: DiagramFont, fontSize: number): number {
	return Math.round(canvasWidth(text, font, fontSize) * 100) / 100;
}
