...
export async function viewport(argv: string[]): Promise<void> {
  ...
  const manual = flags.zoom !== undefined || flags['offset-x'] !== undefined || flags['offset-y'] !== undefined;
  const modes = [flags.fit === true, flags.ids !== undefined, flags.element !== undefined, manual]
  ...
  const result = await setViewport({
    ...
    ...(flags.ids !== undefined
      ? { scrollToElementIds: String(flags.ids).split(',').map(id => id.trim()).filter(Boolean) }
      : {}),
    ...(flags.element !== undefined ? { scrollToElementId: String(flags.element) } : {}),
    ...(flags.zoom !== undefined ? { zoom: number(String(flags.zoom), 'zoom') } : {}),
    ...(flags['offset-x'] !== undefined ? { offsetX: number(String(flags['offset-x']), 'offset-x') } : {}),
    ...(flags['offset-y'] !== undefined ? { offsetY: number(String(flags['offset-y']), 'offset-y') } : {}),
