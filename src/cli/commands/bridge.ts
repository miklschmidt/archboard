...
const bridgeRefusal = {
	...
	description: "The named sources, crossing, style, span, or provenance cannot form the requested bridge.",
};
...
export const bridgeContract = defineCommand({
	...
	parameters: [
		{ kind: "option", key: "over", spellings: ["--over"], value: "required", description: "Connector drawn over the crossing" },
		{ kind: "option", key: "under", spellings: ["--under"], value: "required", description: "Connector drawn under the crossing" },
		{ kind: "option", key: "background", spellings: ["--background"], value: "required", description: "Explicit opaque board background" },
		{ kind: "option", key: "at", spellings: ["--at"], value: "required", description: "Select one crossing by x,y" },
	],
	input: { ingress: BridgeInputSchema },
	result: BridgeResultSchema,
	output: {
		cases: [{ id: "json", when: {}, mode: "json", held: "object-field-and-stderr-note", description: "Created bridge", presentation: ["result", "held-note"] }],
		select: () => "json",
	},
	prerequisites: ["server", "board", "doing"],
	effects: ["write"],
	refusals: [...boardWriteRefusals, bridgeRefusal],
	relationships: [{ method: "POST", path: "/api/bridges", cardinality: "one", description: "Plan and create both bridge parts in one mutation" }],
...
export const bridgeRemoveContract = defineCommand({
	...
	description: "Deletes exactly one strict mask/redraw pair without requiring its source connectors.",
	examples: ['archboard bridge remove Ab12Cd34 --board system --doing "removing crossing marker"'],
	parameters: [{ kind: "positional", key: "bridgeId", name: "bridge-id", description: "Bridge ID (the mask element ID)" }],
	input: { ingress: BridgeRemoveInputSchema },
	result: BridgeRemoveResultSchema,
	output: {
		cases: [{ id: "json", when: {}, mode: "json", held: "object-field-and-stderr-note", description: "Removed bridge", presentation: ["result", "held-note"] }],
		select: () => "json",
	},
	prerequisites: ["server", "board", "doing"],
	effects: ["write"],
	refusals: [...boardWriteRefusals, bridgeRefusal],
	relationships: [{ method: "DELETE", path: "/api/bridges/:id", cardinality: "one", description: "Resolve provenance and delete both parts in one mutation" }],
	async handler(input, context) {
