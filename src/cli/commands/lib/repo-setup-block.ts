...
function renderBlock(options: {
	...
	return [
		...
		"architecture IS — the parts, what contains what, how they are wired — and the",
		"renderer draws it. Nobody moves a box, and there is deliberately no way to.",
		`The commands are in the \`archboard\` skill at \`${skill}\`. Below is the part of`,
		"the setup that only this machine knows.",
		"",
		"### Environment",
		"",
		"A board is one `.semantic.json` document in a vault. This repo uses:",
		"",
		"```bash",
		...env,
		"```",
		...
		"",
		"```bash",
		`ARCHBOARD_VAULT=${vault} ${cli} semantic`,
		"```",
		"",
		"The server keeps the vault it was started with. `status` prints the vault in",
		`use. If that is not the one above, run \`${cli} stop\` and try again with the`,
		"variable set.",
		"",
		"### Running the CLI",
		"",
		...
		"Open that in a browser to read a board. Authoring and reading a board from the",
...
async function chooseVault(options: SetupOptions, repo: string): Promise<string> {
	...
	const answer = await options.context.prompt(
		"Where should this repo keep its boards? (a vault directory, shared or local)",
		suggested,
	);
