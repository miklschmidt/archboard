...
import { isCoordinatorOwned, planQueueReorder } from "../adapter.ts";
import type { WorkbenchQueueEntry, WorkbenchQueueReorderMove } from "../contract.ts";
import { entry, submission, type EntrySeed } from "./support.ts";

/** c = coordinator-owned, f = foreign. */
function entries(shape: string): WorkbenchQueueEntry[] {
	return [...shape].map((mark, index) =>
		entry({
			id: `${mark}${index + 1}`,
			operationId: mark === "f" ? null : undefined,
		} satisfies EntrySeed),
	);
}
...
function order(
	...
	return plan.moved ? plan.orderedSubmissionIds.map(String) : plan.reason;
}
...
describe("coordinator ownership", () => {
	test("an entry with no coordinator operation is foreign", () => {
		...
		expect(isCoordinatorOwned(entry({ id: "b", operationId: null }))).toBe(false);
	});
});
...
describe("queue reorder planning", () => {
	test("submits every submission id, not just the moved one", () => {
		...
		if (!plan.moved) throw new Error("expected a plan");
		expect(plan.orderedSubmissionIds).toHaveLength(3);
		expect([...plan.orderedSubmissionIds].map(String).toSorted()).toEqual(["c1", "c2", "c3"]);
	...
	test("preserves the relative order of every foreign entry", () => {
		...
		if (!Array.isArray(moved)) throw new Error("expected a plan");
		const foreignAfter = moved.filter((id) => id.startsWith("f"));
		expect(foreignAfter).toEqual(before);
	...
	test("never mutates the authoritative entries it was given", () => {
		...
		const before = authoritative.map((item) => String(item.submissionId));

		planQueueReorder(authoritative, submission("c1"), STEP_LATER);

		expect(authoritative.map((item) => String(item.submissionId))).toEqual(before);
	});
});
