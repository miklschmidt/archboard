...
import { mediaSnapshot } from "@/ui/voice-session/tests/support/fakes";
import { harness, listening } from "@/ui/voice-session/tests/support/harness";

/**
 * The adapter's half of the mute path. `muted` is reachable from `listening`
 * alone and `listening` from `muted` alone, so the whole contract is that the
 * offer follows the phase and a command is never sent for an offer that was
 * not made.
 */
describe("voice session mute", () => {
	...
	test("keeps the binding and the session identity across a mute", async () => {
		const { session, realtime } = harness("pane-a");
		await session.start();
	...
	test("treats a toggle whose phase moved under it as a silent no-op", async () => {
		...
		// The real media session never rejects a mute: for any phase but
		// `listening` it returns the run unchanged. That is what a press losing
		// the race against a phase change looks like from here.
		realtime.onMute(() => {
			const current = realtime.snapshot();
			if (current === null) {
				throw new Error("The fake has no snapshot.");
			}
	...
	test("surfaces a control this adapter drove and had refused", async () => {
		const { session, realtime } = harness();
		realtime.set(listening());
		realtime.onStop(() => Promise.reject(new Error("The realtime host refused the stop.")));
	...
	test("drops a refused control the moment the run moves past it", async () => {
		...
		// alert and the offered retry would ride every later phase of a session
		// that has gone on working.
