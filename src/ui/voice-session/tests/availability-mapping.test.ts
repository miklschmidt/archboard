...
import { projectVoiceSession } from "@/ui/voice-session";
import type { VoiceSessionProjectionInput, VoiceSessionView } from "@/ui/voice-session";
import {
	capabilities,
	connectedState,
	mediaSnapshot,
...
const GATES: readonly [string, VoiceSessionProjectionInput, string][] = [
	[
		"reconnecting",
		input({ transportState: reconnectingState() }),
		"The Codex workbench connection dropped.",
	],
	[
		"attaching",
	...
	[
		"unbound link",
		input({ transportState: connectedState(snapshot({ threadLink: UNBOUND_LINK })) }),
	...
	[
		...
		input({
			transportState: connectedState(
				snapshot({ coordinator: coordinator({ state: "starting", threadId: null, reason: null }) }),
			),
		}),
	...
	[
		...
		input({
			transportState: connectedState(
				snapshot({ voice: voice({ state: "unavailable", reason: "Voice is off for this pane." }) }),
			),
		}),
...
describe("voice session availability mapping", () => {
	test.each(GATES)("names the %s gate that blocks a start", (_name, candidate, detail) => {
		const view = projectVoiceSession(candidate);
		expect(view.status).toBe("unavailable");
		expect(view.detail).toBe(detail);
		expect(view.controls.canStart).toBe(false);
		expect(view.accessibleStatus).toBe(`Voice unavailable. ${detail}`);
	});
	...
	] as const)("presents the %s owner unavailability as a %s failure", (reason, code) => {
	...
	test.each(["detached", "socket_closed"] as const)(
		"presents %s as plain unavailability with no failure",
		(reason) => {
			const view = projectVoiceSession(
				input({ mediaState: { state: "unavailable", reason, message: `${reason} happened.` } }),
			);
