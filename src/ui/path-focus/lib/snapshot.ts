// The shapes of path focus and their comparisons. Pure: no React, no DOM, and
// nothing here writes the board.
...
interface NoPathFocus {
	...
	/** The selected element, or null when the reason is `empty` or `multiple`. */
...
interface ConnectedPathFocus {
	kind: "connected";
	selectedId: string;
	/** Every element on the path, the selected one included, in id order. */
