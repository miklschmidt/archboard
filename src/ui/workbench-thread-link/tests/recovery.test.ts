...
import { createThreadLinkController, projectThreadLinkReadiness } from "@/ui/workbench-thread-link";
import type {
...
const EVERY_HOST_INTENT: readonly ThreadLinkRecoveryIntent[] = [
	"start_workbench",
	"choose_binary",
	"unlock_home",
	"repair_storage",
...
/**
 * The readiness disclosure of one state.
...
function readiness(
	...
	hostRecoveryIntents: readonly ThreadLinkRecoveryIntent[] = EVERY_HOST_INTENT,
): ThreadLinkReadinessDisclosure {
	return projectThreadLinkReadiness({ state, capabilities: capabilities(), hostRecoveryIntents });
}

/**
 * A connected state with one readiness.
 * @param value The readiness.
 * @returns The state.
 */
function withReadiness(value: BrowserSnapshot["readiness"]): WorkbenchTransportState {
	return connected(snapshot({ readiness: value }));
}

/**
 * The recovery intents one state offers.
 * @param state The transport state.
 * @returns The intents.
 */
function intents(state: WorkbenchTransportState): readonly string[] {
...
describe("thread-link readiness disclosure", () => {
	test("a missing or wrong Codex binary is one arm carrying the host's own reason", () => {
		...
		expect(disclosed.recoveries.every((recovery) => recovery.available)).toBe(true);
	});
	...
	test("a stale snapshot names both sequences and offers a refresh", () => {
		const state: WorkbenchTransportState = {
			kind: "stream",
	...
	test("a disconnected pane still discloses the connection arm it is in", () => {
		const disconnected: WorkbenchTransportState = {
			kind: "connection",
			state: "stopped",
			connection: "stopped",
			snapshot: null,
...
describe("thread-link recovery actions", () => {
	...
	test("a declared host recovery runs against the captured pane and intent", async () => {
		...
		const controller = createThreadLinkController({
			capturePane: capturing(transport, ["unlock_home"]),
			captureHostRecovery: hostOwner("pane-a", (target) => {
	...
	test("a host authority for another pane is refused rather than run", async () => {
		const transport = new FakeTransport();
		const controller = createThreadLinkController({
			capturePane: capturing(transport, ["repair_storage"]),
			captureHostRecovery: hostOwner("pane-b", () =>
				Promise.reject(new Error("This recovery must never run.")),
			),
		});
	...
	test("a failing host recovery reports what happened and what to do next", async () => {
		const transport = new FakeTransport();
		const controller = createThreadLinkController({
			capturePane: capturing(transport, ["start_workbench"]),
			captureHostRecovery: hostOwner("pane-a", () =>
				Promise.reject(new Error("The Codex process refused to start.")),
			),
		});
		await controller.recover("start_workbench");
		const settled = controller.snapshot();
