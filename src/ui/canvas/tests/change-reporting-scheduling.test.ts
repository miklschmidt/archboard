...
import { REPORT_IDLE_SETTLE_MS, REPORT_PROGRESS_MS } from "@/shared/timing/timing";
import { hasPendingEdits } from "@/ui/canvas/change-reporting";
import { ReportingHarness, copy, find } from "@/ui/canvas/tests/support/change-reporting-harness";
...
describe("delivery around server scene application", () => {
	test("a due delivery waits during application and drains after the final completion", () => {
		const harness = new ReportingHarness();
		harness.edit("a", { x: 10 });
		harness.applyServerElements([{ ...find(harness.server.document, "b"), y: 30 }]);
	...
	test("a user edit during server application schedules once completion runs", () => {
		const harness = new ReportingHarness();
		harness.applyServerElements([{ ...find(harness.scene, "b"), y: 30 }]);
...
describe("empty deadlines", () => {
	test("a correction followed by an empty idle deadline checks settled release", () => {
		...
		harness.clock.advance(200);
		harness.accept({
			upserts: [{ ...find(harness.server.document, "a"), x: 21 }],
			deletes: [],
		});
