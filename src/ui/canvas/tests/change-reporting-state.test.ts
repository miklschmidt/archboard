...
import * as reporting from "@/ui/canvas/change-reporting";
import {
	hasPendingEdits,
...
/**
 * An image file as Excalidraw holds it.
 * @param id The file id.
 * @param dataURL Its bytes.
 * @param created When it was added.
 * @returns The file.
 */
function imageFile(id: string, dataURL: string, created: number): BinaryFileData {
	// Excalidraw brands file ids and data URLs; a test names plain strings.
	// oxlint-disable-next-line typescript/no-unsafe-type-assertion
	return { id: id as FileId, dataURL: dataURL as DataURL, mimeType: "image/png", created };
}
...
describe("public reporting state", () => {
	...
	test("frontend source tags are bookkeeping while human upserts keep scene order", () => {
		...
		expect(
			harness.server.requests[0]?.report.upserts.every(
				(element) => element["source"] === undefined,
			),
		).toBe(true);
	});
});
...
describe("file and incoming server frames", () => {
	test("a replacement file frame removes stale membership and replaces reused-id bytes", () => {
		 ...
		 * @returns The record.
		 */
		function getFiles(): Record<string, BinaryFileData> {
			return files;
		}
		/**
		 * Excalidraw's additive add.
		 * @param incoming The files to add.
		 */
		function addFiles(incoming: BinaryFileData[]): void {
			additiveCalls += 1;
			for (const file of incoming) {
				files[file.id] ??= file;
			}
		}
		replaceCanvasFiles({ getFiles, addFiles }, [
	...
	test("incoming updates and deletes never overwrite a locally dirty id", () => {
		const harness = new ReportingHarness();
		harness.edit("a", { x: 70 });
		harness.applyServerElements([{ ...find(harness.server.document, "a"), x: 25 }]);
		harness.clock.advance(0);
		expect(find(harness.scene, "a").x).toBe(70);
		...
		deleted.remove("a");
		deleted.applyServerElements([{ ...find(deleted.server.document, "a"), x: 25 }]);
	...
	test("overlapping server updates retain ordered completion records", () => {
		const harness = new ReportingHarness();
		harness.applyServerElements([{ ...find(harness.scene, "a"), x: 5 }]);
		harness.applyServerElements([{ ...find(harness.scene, "b"), y: 5 }]);
