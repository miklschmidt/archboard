...
import { REPORT_RETRY_MS } from "@/shared/timing/timing";
import { needsFullReport, reportsSettled } from "@/ui/canvas/change-reporting";
import { ownsHoldAttempt } from "@/ui/canvas/hold-attempt";
import {
	ReportingHarness,
	ScriptedServer,
	box,
...
describe("refusal and retry", () => {
	...
	test("a failed request waits exactly 2,000 ms and retries the same delta", () => {
		...
		if (first === undefined) {
			throw new Error("The edit did not produce a report");
		}
