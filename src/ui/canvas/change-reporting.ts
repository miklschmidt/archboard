...
function beginReport(
	...
	if (
		state.inFlightReport !== null ||
		(state.applyingServerUpdateCount > 0 && !allowWhileApplyingServerUpdate)
	) {
...
export function reduce(state: ChangeReportingState, event: ChangeReportingEvent): ReduceResult {
	...
	switch (event.type) {
		...
		case "report_failed":
			if (event.generation !== state.generation) return { state, effects };
			return {
				state: afterFailedReport(state, effects, {
