// The words of the approvals surface: family titles and kickers, phase
// labels, the host's spoken-eligibility vocabulary in human words, and the
// transport's refusals.
...
const DYNAMIC_KICKER = "Dynamic coordination approval";
...
const TERMINAL_PHASES: ReadonlySet<WorkbenchApprovalPhase> = new Set([
	"approved",
	"declined",
	"cancelled",
	"expired",
...
} as const satisfies Record<BrowserApproval["spoken"]["reason"], string>;

const DYNAMIC_SPOKEN_DETAIL =
...
const IMMUTABLE_EFFECT_NOTICE =
	"The effect and its hash are fixed. Approving sends exactly this effect, or nothing.";

const TRANSPORT_REFUSALS: ReadonlyMap<string, string> = new Map([
	[
		"approval_not_pending",
		"This approval is no longer pending for the target it was raised against.",
	],
	[
	...
	[
		...
		"The host answered with a workbench contract this browser cannot read.",
	],
	["response_lost", "The host never answered, so the decision's outcome is unknown."],
]);

const GENERIC_REFUSAL = "The host refused this decision.";

/**
 * The words for a refusal code.
 * @param code The transport or gateway code.
 * @returns The refusal in human words.
 */
function refusalMessage(code: string): string {
	return TRANSPORT_REFUSALS.get(code) ?? GENERIC_REFUSAL;
}

/**
 * The words for the host's spoken-eligibility reason.
 * @param reason The host's reason.
 * @returns The detail.
 */
function spokenDetail(reason: BrowserApproval["spoken"]["reason"]): string {
