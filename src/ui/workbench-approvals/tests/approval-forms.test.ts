...
/**
 * The fields of an elicitation with one field.
 * @param overrides The field's facts.
 * @returns The fields.
 */
...
/**
 * The first error one answer produces.
 * @param overrides The field's facts.
...
describe("reviewed field descriptors", () => {
	...
	test("carries a requestUserInput question with its options and other answer", () => {
		const [environment] = approvalFields(userInputApproval());

		expect(environment?.name).toBe("question:environment");
		expect(environment?.control).toBe("multi_enum");
		expect(environment?.allowsOther).toBe(true);
		expect(environment?.options?.map((option) => option.label)).toEqual(["staging", "production"]);
	});

	test("carries a secret requestUserInput question as a secret control", () => {
		const [, token] = approvalFields(userInputApproval());

		expect(token?.name).toBe("question:token");
		expect(token?.secret).toBe(true);
		expect(token?.control).toBe("secret");
	});
...
describe("reviewed field validation", () => {
	...
	test("holds item bounds and the offered answers of a multi-choice field", () => {
		...
		/**
		 * The errors one selection produces.
		 * @param value The chosen options.
		 * @returns The errors.
		 */
...
describe("genuine binary approvals", () => {
	...
	test("refuses a broader grant, an amendment, an incomplete set and every other family", () => {
		const sets: readonly (readonly unknown[])[] = [
			["accept"],
