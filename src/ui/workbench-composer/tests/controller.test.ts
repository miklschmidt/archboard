...
import { createWorkbenchComposerController } from "@/ui/workbench-composer";
import {
	OTHER_THREAD,
	OTHER_TURN,
	THREAD,
...
/**
 * A result the host lost.
 * @returns The result.
 */
function unknownOutcome(): ReturnType<typeof commandResult> {
	return commandResult({ outcome: "outcome_unknown", code: null });
}

/**
 * A result the host refused as not ready.
 * @returns The result.
 */
function notReady(): ReturnType<typeof commandResult> {
	return commandResult({ outcome: "not_delivered", code: "not_ready", message: null });
}
...
/** The resolver a gate holds before its promise has captured the real one. */
...
/**
 * An answer that delivers once a gate opens.
 * @param gate The gate.
 * @returns The answer.
 */
function releasedBy(gate: Gate): CommandAnswer {
	return async () => {
		await gate.promise;
 ...
 * @returns The error.
 */
function lostResponse(): BrowserWorkbenchTransportError {
	return new BrowserWorkbenchTransportError("response_lost", "lost", {
		outcome: "outcome_unknown",
	});
}

/**
 * A transport that moves its own state before answering.
 * @param transport The double.
 * @param next The state it moves to.
 * @param answer The answer it then gives.
 */
function movingBeforeAnswer(
	transport: FakeComposerTransport,
	next: ReturnType<typeof connected>,
	answer: CommandAnswer,
): void {
	transport.setCommand((recorded) => {
...
describe("what the composer sends, and what it names", () => {
	test("a start command carries the intent captured at activation", async () => {
	...
	test("a steer command carries the captured intent and the authoritative turn", async () => {
	...
	test("an explicit queue sends queueAdd with the captured intent and reports queued", async () => {
		const transport = fakeComposerTransport({ state: RUNNING });
		const controller = createWorkbenchComposerController({ transport });
		const result = await controller.submit({ text: "Then label it.", delivery: "queue" });
		expect(transport.sent[0]?.draft).toEqual({ command: "queueAdd", prompt: "Then label it." });
		expect(transport.sent[0]?.target).toEqual(commandIntent());
		expect(result).toEqual({ outcome: "queued" });
		expect(controller.getState().retained).toBeNull();
	});

	test("an interrupt command carries the captured intent and its captured turn", async () => {
		const transport = fakeComposerTransport({ state: RUNNING });
		const controller = createWorkbenchComposerController({ transport });
...
describe("the authoritative turn a delivered command may claim", () => {
	...
	test("a start without an accepted turn identity remains unknown", async () => {
		const { turnId, ...withoutTurn } = commandResult();
		const transport = fakeComposerTransport();
		transport.setCommand(answering(withoutTurn));
		const controller = createWorkbenchComposerController({ transport });
...
describe("the pending command lease", () => {
	test("a command in flight is published as pending and blocks a second submit", async () => {
		const gate = deferred();
		const transport = fakeComposerTransport();
		transport.setCommand(releasedBy(gate));
		const controller = createWorkbenchComposerController({ transport });
...
describe("the draft policy for each settled outcome", () => {
	...
	test("not_delivered keeps the draft in the live composer and retains no copy", async () => {
		const transport = fakeComposerTransport();
		transport.setCommand(answering(notReady()));
		const controller = createWorkbenchComposerController({ transport });
	...
	test("outcome_unknown retains an inert copy naming its own thread", async () => {
		const transport = fakeComposerTransport();
		transport.setCommand(refusing(lostResponse()));
		const controller = createWorkbenchComposerController({ transport });
	...
	test("the retained copy is dismissable and nothing resends it", async () => {
		const transport = fakeComposerTransport();
		transport.setCommand(answering(unknownOutcome()));
		const controller = createWorkbenchComposerController({ transport });
...
describe("refusals, disconnects, and late results", () => {
	test("the transport's link_changed refusal is reported in the composer's words", async () => {
		const transport = fakeComposerTransport();
		transport.setCommand(refusing(new BrowserWorkbenchTransportError("link_changed", "moved")));
		const controller = createWorkbenchComposerController({ transport });
		expect(await controller.submit({ text: "Go." })).toEqual({
			outcome: "not_delivered",
	...
	test("a disconnect before the command is sent is not delivered", async () => {
		const transport = fakeComposerTransport();
		transport.setCommand(
			refusing(new BrowserWorkbenchTransportError("socket_unavailable", "gone")),
		);
		const controller = createWorkbenchComposerController({ transport });
		expect(await controller.submit({ text: "Go." })).toEqual({
			outcome: "not_delivered",
...
describe("remediations from the fixed-range review", () => {
	test("a definitive not_delivered is not relabelled unknown by a relink in flight", async () => {
		...
		movingBeforeAnswer(
			...
			refusing(new BrowserWorkbenchTransportError("link_changed", "moved")),
		);
		const controller = createWorkbenchComposerController({ transport });
		const result = await controller.submit({ text: "Rename the node." });
	...
	test("a retained copy stands through a later command and goes on a delivered resend", async () => {
		...
		transport.setCommand(answering(commandResult()));
		await controller.submit({ text: "Rename the node." });
		expect(controller.getState().retained).toBeNull();
	});
