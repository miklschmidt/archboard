...
export interface AgentAudioVisualizerWaveProps {
	...
	/**
	 * The voice output state. Archboard adaptation: replaces LiveKit's `AgentState`.
	...
	blur?: number;
	/**
	 * Measured model output level (0-1). Archboard adaptation: replaces the
	 * LiveKit `audioTrack`; there is no microphone input path.
	 */
	level: number;
	/**
	 * Whether the session is live. False stops the animation loop.
	 */
	active: boolean;
	/**
	 * Whether motion is reduced; the loop then rests at the state's static values.
	 */
...
export function AgentAudioVisualizerWave({
	...
	const { speed, amplitude, frequency, opacity } = useVoiceOutputWave({
		state,
		level,
		active,
		reducedMotion,
