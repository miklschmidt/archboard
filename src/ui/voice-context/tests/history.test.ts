...
import { createVoiceContextHistory } from "@/ui/voice-context";
import type { VoiceContextLedgerEntry } from "@/ui/voice-context";
import {
	BINDING_A,
	BINDING_B,
	SESSION_A,
...
/**
 * An entry that claims not to have been attempted while carrying attempt evidence.
 * @param id The entry id.
 * @param attemptedAtMs The contradictory attempt time.
 * @param outcome The contradictory outcome.
 * @returns The incoherent entry.
 */
function unattemptedWith(
	id: string,
	attemptedAtMs: number | null,
	outcome: VoiceContextLedgerEntry["outcome"],
): VoiceContextLedgerEntry {
	const contradictory = { ...ledgerEntry(id, { attempted: false }), attemptedAtMs, outcome };
	// oxlint-disable-next-line typescript/no-unsafe-type-assertion -- The history validates untrusted wire evidence at runtime; this contradiction is exactly the shape the ledger entry type forbids, so the only way to hand it over is to assert it.
	return contradictory as VoiceContextLedgerEntry;
}
...
describe("voice context history", () => {
	...
	test("retains externally observed replaced and stopped views without owning transitions", () => {
		...
		const oldSession = defined(history.snapshot().sessions[0]);
		const newSession = defined(history.snapshot().sessions[1]);
		expect(defined(oldSession.observations.at(-1)).session.failure?.code).toBe("replaced");
		expect(oldSession.entries.map((entry) => entry.id)).toEqual(["1"]);
		expect(defined(newSession.observations.at(-1)).session.status).toBe("stopped");
	...
	test("retains disconnected, uncertain, and recovered delivery evidence", () => {
		...
		expect(defined(history.snapshot().sessions[0]).entries).toEqual([uncertain]);
	});
	...
	test("accepts only identity values carrying the fitter's clipped-prefix evidence", () => {
		const clipped: Record<string, unknown> = JSON.parse(
			canonicalBrief(SESSION_A, { truncated: true }),
		);
	...
	test("rejects incoherent timing, outcomes, and bodies before retention", () => {
		const history = createVoiceContextHistory();
		history.capture(capture());
		const invalid: readonly VoiceContextLedgerEntry[] = [
			ledgerEntry("bad-capture", { freshness: { capturedAtMs: Number.NaN, freshUntilMs: 2 } }),
			ledgerEntry("bad-window", { freshness: { capturedAtMs: 10, freshUntilMs: 9 } }),
			ledgerEntry("bad-attempt-order", {
				freshness: { capturedAtMs: 10, freshUntilMs: 20 },
				attemptedAtMs: 9,
			}),
			unattemptedWith("bad-unattempted-outcome", null, "outcome_unknown"),
			unattemptedWith("bad-unattempted-time", 12, "not_delivered"),
			ledgerEntry("bad-body", { body: "x".repeat(BROWSER_VOICE_CONTEXT_BODY_MAX_UTF8_BYTES + 1) }),
		];
		for (const entry of invalid) {
			expect(history.append({ session: SESSION_A, entry }), entry.id).toMatchObject({
				outcome: "ignored",
				reason: "invalid_delivery_evidence",
			});
		}
		expect(defined(history.snapshot().sessions[0]).entries).toHaveLength(0);

		const notAttempted = ledgerEntry("valid-no-attempt", { attempted: false });
