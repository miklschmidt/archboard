...
describe("voice context projection", () => {
	test("derives every baseline field from the exact canonical copy bytes", () => {
		...
		expect(fields).toMatchObject({
			...
			Board: "Checkout",
			"Board key": "checkout",
			"Board document": "Architecture/checkout.semantic.json",
			Pane: "primary",
			"Focused at capture": "Focused",
			Version: "42",
			Variant: "Current (current)",
			View: "Whole variant",
			Selection: "node api, node worker",
			"Selected ids": "api, worker",
			Differences: "No predecessor",
			Reconciliation: "Nothing to settle",
			Claim: "agent",
