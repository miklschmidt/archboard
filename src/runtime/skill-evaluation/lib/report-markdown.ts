...
const COLUMNS = [
	...
	"completeness",
	"missed",
	"findings conf./unseen/truth/skill/uncited",
	"what moved",
] as const;
...
function armLine(key: string, arm: Arm, s: ArmSummary): string {
	return line({
		...
		"findings conf./unseen/truth/skill/uncited": `${s.findings.conformance}/${s.conformanceUnseen}/${s.findings.truth}/${s.findings.skill}/${s.uncited}`,
	});
}
...
function disagreementLine(split: SkillDisagreement): string {
	return `- ${split.scenario} ${split.feature}: counted as untaught on ${split.untaught.join(", ")}; counted against the run on ${split.otherwise.join(", ")}`;
}

/**
 * One line about a skill finding that named a passage its feature cites.
 * @param excused The finding, with its run.
 * @returns The markdown line.
 */
function excusedDepartureLine(excused: ExcusedDepartureOnRun): string {
	return `- ${excused.run} (${excused.arm}, ${excused.scenario} rep ${excused.repetition}): ${excused.feature}, at ${excused.passage}: ${excused.gap}`;
}
...
function renderReportMarkdown(report: Report): string {
	return [
		...
		"",
		"### Features counted as untaught on some runs of a scenario and against the run on others",
		"",
		...listed(report.skillDisagreements, disagreementLine),
		"",
		"## Findings put on the skill that name a passage the feature cites (counted as departures from the skill; each fails its run)",
		"",
		...listed(report.excusedDepartures, excusedDepartureLine),
		"",
		...concernLines(report),
