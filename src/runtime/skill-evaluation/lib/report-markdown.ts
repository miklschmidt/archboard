...
function disagreementLine(split: SkillDisagreement): string {
	return `- ${split.scenario} ${split.feature}: untaught on ${split.untaught.join(", ")}; held against the run on ${split.otherwise.join(", ")}`;
}
...
function renderReportMarkdown(report: Report): string {
	return [
		...
		"",
		"### Features the grader called untaught on some runs of a scenario and held against the run on others",
		"",
