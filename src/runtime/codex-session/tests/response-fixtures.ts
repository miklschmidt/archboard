...
export function responseFor(method: ResponseMethod, fixture: SessionFixture): unknown {
	...
	if (method === "configRequirements/read") {
		return requirementsFixture(null);
	}
	...
	if (method === "model/list") {
		return { data: [modelFixture], nextCursor: "model-next" };
	}
	...
	if (method === "turn/start" || method === "thread/queue/start") {
		return { turn: turnFixture };
	}
	...
	if (method === "thread/loaded/list") {
		return { data: ["thread-1"], nextCursor: "loaded-next" };
	}
