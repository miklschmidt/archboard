	...
	return (event) => {
		...
		if (notification.method === "turn/started" || notification.method === "turn/completed") {
			...
			for (const state of states) {
				...
				if (!matchesClient && !matchesTurn) {
					continue;
				}
				if (state.turnId !== null && !matchesTurn) {
					continue;
				}
		...
		if (
			...
			for (const state of states) {
				...
				if (state.outcome === "pending" || state.outcome === "outcome_unknown") {
					...
					if ((state.outcome as string) !== "delivered") {
						continue;
					}
				}
		...
		if (
			...
			if (!notification.params.status.activeFlags.some((flag) => ATTENTION_FLAGS.has(flag))) {
				return;
			}
			for (const state of states) {
