...
function writeTypeOwner(
	...
	writeFileSync(
		...
		[
			...
			"\t\t\t: never",
			"\t: never;",
			"type JsonArrayElements<Value> = JsonArrayBranches<Value> extends infer Branch",
			"\t? Branch extends readonly (infer Element)[]",
			"\t\t? Element",
			"\t\t: never",
			"\t: never;",
			"type HasStringIndex<Value> = Value extends object",
			"\t? string extends keyof Value",
			"\t\t? true",
			"\t\t: false",
			"\t: false;",
			"type JsonObjectBranchesHaveStringIndexes<Value> = EveryCheckPasses<",
			"\tJsonObjectBranches<Value> extends infer Branch",
			"\t\t? Branch extends unknown",
			"\t\t\t? HasStringIndex<Branch>",
			"\t\t\t: never",
			"\t\t: never",
			">;",
			"type JsonObjectIndexValues<Value> = JsonObjectBranches<Value> extends infer Branch",
			"\t? Branch extends object",
			"\t\t? string extends keyof Branch",
			"\t\t\t? StringIndexValue<Branch>",
			"\t\t\t: never",
			"\t\t: never",
			"\t: never;",
			...
			"\t\t\t\t? false",
			"\t\t\t\t: IsAny<JsonArrayElements<Value>> extends true",
			"\t\t\t\t\t? false",
			"\t\t\t\t\t: SameType<JsonArrayElements<Value>, Value> extends true",
			"\t\t\t\t\t\t? IsNever<JsonObjectBranches<Value>> extends true",
			"\t\t\t\t\t\t\t? false",
			"\t\t\t\t\t\t\t: JsonObjectBranchesHaveStringIndexes<Value> extends true",
			"\t\t\t\t\t\t\t\t? IsAny<JsonObjectIndexValues<Value>> extends true",
			"\t\t\t\t\t\t\t\t\t? false",
			"\t\t\t\t\t\t\t\t\t: SameType<Exclude<JsonObjectIndexValues<Value>, undefined>, Value>",
			"\t\t\t\t\t\t\t\t: false",
			"\t\t\t\t\t\t: false",
			"\t\t\t\t\t: false;",
