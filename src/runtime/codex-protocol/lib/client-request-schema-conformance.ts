...
function writeTypeOwner(
	...
	writeFileSync(
		...
		[
			...
			"\t\t: IsAny<Right> extends true",
			"\t\t\t? false",
			"\t\t\t: (<Probe>() => Probe extends Left ? 1 : 2) extends <Probe>() =>",
			"\t\t\t\t\tProbe extends Right ? 1 : 2",
			"\t\t\t\t? (<Probe>() => Probe extends Right ? 1 : 2) extends <Probe>() =>",
			"\t\t\t\t\t\tProbe extends Left ? 1 : 2",
			"\t\t\t\t\t? true",
			"\t\t\t\t\t: false",
			"\t\t\t\t: false;",
			...
			"\t\t: never",
			">;",
			"type PureRequiredJsonObject<Value> = Record<string, Value>;",
			"type PureOptionalTsRsJsonObject<Value> = { [Key in string]?: Value };",
			"// Compare the complete branch. Mapping keyof Branch would erase explicit named keys.",
			"type JsonObjectBranchHasPureShape<Branch, Value> =",
			"\tExactType<Branch, PureRequiredJsonObject<Value>> extends true",
			"\t\t? true",
			"\t\t: ExactType<Branch, PureOptionalTsRsJsonObject<Value>>;",
			...
			"\t\t\t\t? false",
			"\t\t\t\t: SameType<Exclude<StringIndexValue<Branch>, undefined>, Value> extends true",
			"\t\t\t\t\t? JsonObjectBranchHasPureShape<Branch, Value>",
			"\t\t\t\t\t: false",
			"\t\t\t: false",
			"\t\t: false;",
