...
// finished. A user interrupting simply delays that next request.
//
...
function voicePresentationPrompt(name: string): string {
	return [
		...
		"Explain each step as a presenter would: what it shows and why it matters, in your own words and their language, with names and never ids. Never say a step's number or the total aloud: their screen shows where they are, and hearing it every step is tiring. The number you are handed is how you know which step is the last; never count the steps yourself or guess how many there are.",
...
function coordinatorPresentationInstructions(name: string): string {
	return [
		`Walkthrough presentation mode. The voice model is presenting the walkthrough "${presentationName(name)}" to the user as a talk and paces it: it asks you to present a step, explains the step aloud, and asks for the next when it has finished. The user is listening to silence while you work, so a request to present a step is answered as fast as you can.`,
		"Every request you receive in this mode is a request to present the next step unless its transcript shows the user asking for something else. A request arrives carrying the latest thing said as the user, replayed, never words the voice model chose, so until the user speaks it repeats their opening request: to be given the talk one step at a time, moving on to the next step after each. That is what it means every time it arrives; never take it as a reason to present step 1 again, and read the transcript beside it for anything the user has asked since. Call the present_step tool as your very first action with no arguments: the host knows which step comes next, including after the user moved by hand. Call it once. It resolves to one string of JSON holding the whole step, so print it as it is with text(result) and read the step from that; a script that looks for a content array prints nothing, and the user waits in silence through every retry. Pass a step number only when the user asked for a particular step. This overrides every instruction to prepare first: do not read a skill, run a command, consult a task tracker, inspect a board or write a preamble message before calling it. The walkthrough is already known to the host, and the pane, board and variant are the host's. The tool answers only once the step has finished arriving on the user's screen.",
		...
		"If present_step is refused, do not describe the step. Tell the voice model plainly what the refusal says, such as the user stepping by hand or the pane having closed.",
		"A developer message may tell you the user moved the presentation by hand or left it. Follow the picture: the next step asked for continues from where the user is, and nothing is presented after they leave unless they ask again.",
...
/** What a user did to a presented walkthrough by hand. */
...
/**
 * What both models are told when a user moves a presentation by hand, or leaves it.
 *
 ...
 * a step the user chose is handed to the voice model as speech, in the words the coordinator
...
function presentationChangeTexts(change: RealtimePresentationChange): {
	...
	if (change.kind === "left") {
		const text =
			"The user left the walkthrough presentation (data). Stop presenting: no further step is asked for or presented unless the user asks again.";
	...
	return {
		coordinator: `The user moved the walkthrough presentation by hand to ${where} (data). That step is on their screen now; the narration continues from it.`,
		voice: {
			via: "speech",
			text: `The user moved the presentation by hand, and this is on their screen now. Step ${change.step} of ${change.of}, ${change.heading}. ${body}`,
		},
	};
}
