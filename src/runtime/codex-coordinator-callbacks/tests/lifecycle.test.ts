...
describe("coordinator callback lifecycle", () => {
	...
	test("kept retained state prevents duplicate subscriptions across repository reload generations", () => {
		const h = harness(false);
		const retained = kept<CoordinatorCallbacksRetainedState>(
			`coordinator-callback-test-${crypto.randomUUID()}`,
			() => ({ current: h.callbacks }),
		);
		const reloaded = installCodexCoordinatorCallbacks(retained, h.options);
		expect(reloaded).toBe(h.callbacks);
		expect(h.operations.listenerCount()).toBe(1);
		close(h);
	});
});
