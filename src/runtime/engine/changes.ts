...
function identityPairs(before: ServerElement[], after: ServerElement[]): IdentityPair[] {
	...
	for (const [previousNode, byNew] of candidates) {
		...
		const ranked = [...byNew.entries()].toSorted((a, b) => b[1].length - a[1].length);
...
export function headlineFor(change: SemanticChange): string {
	...
	if (n.identity.length > 0) {
		...
		const verb =
			first.what === "promoted" ? "promoted" : first.what === "demoted" ? "demoted" : "renamed";
...
export function narrateChange(change: SemanticChange, maxChars = 1800): string {
	...
	for (const id of change.nodes.identity) {
		if (id.what === "promoted") {
			...
			lines.push(
				`promoted ${quoted(id.to.name)}${id.to.kind ? ` to a ${id.to.kind}` : ""}${id.to.binding ? ` bound to ${id.to.binding}` : ""} (${was})`,
			);
	...
	for (const node of change.nodes.added) {
		lines.push(
			`new ${node.kind ?? "node"} ${quoted(node.name)}${node.binding ? ` bound to ${node.binding}` : ""}`,
		);
	}
	...
	for (const edge of change.edges.added) {
		lines.push(
			`new edge ${quoted(edge.fromName)} → ${quoted(edge.toName)}${edge.label ? ` ("${edge.label}")` : ""}`,
		);
	}
	...
	for (const cl of change.layout.clusters.slice(0, 2)) {
		...
		lines.push(
			`cluster ${cl.kind}${parts.length ? `: ${parts.join(", ")}` : ""}${cl.sharedMembers.length ? ` (around ${namedList(cl.sharedMembers)})` : ""}`,
		);
	}
	...
	for (const g of change.layout.groups) {
		lines.push(
			`group ${g.kind}${g.joined.length ? `: +${namedList(g.joined)}` : ""}${g.left.length ? `: -${namedList(g.left)}` : ""}`,
		);
	}
