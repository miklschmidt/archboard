...
function agentStatement(input: LegacyElementIngress): LegacyElementIngress {
...
function completeElement(input: LegacyElementIngress): RuntimeBoardElement {
...
function completeElements(inputs: LegacyElementIngress[]): RuntimeBoardElement[] {
	return expandElements(inputs.map(agentStatement), { deterministic: true, forStore: true });
}

export { agentStatement, completeElement, completeElements };
