...
//
// The canvas imports this file and reads none of it. The CLI process that reads
...
async function syncBatchToCanvas(
	...
	try {
		...
		logger.debug("Canvas batch sync succeeded", result);
		return result;
	} catch (error) {
		logger.warn("Canvas batch sync failed:", (error as Error).message);
	...
	if (!identityProbe) {
		identityProbe = (async () => {
			try {
				...
				try {
					response = await fetch(`${EXPRESS_SERVER_URL}/health`, {
