...
function buildGpos(buf: Buffer): Kerning {
	...
	for (const index of lookupsForFeatures(buf, h, ["kern"])) {
		...
		for (const sub of lookup.subs) {
			...
			if (type !== 2) {
				continue;
			} // single positioning does not move a pair
	...
	return {
		...
		apply(glyphs, i) {
			...
			for (const ligature of list) {
				...
				if (matched) {
					return { glyphs: [ligature.glyph], consumed: 1 + ligature.components.length };
				}
			}
			return null;
		},
...
function chainContextSubst(
	...
	if (format === 1) {
		...
		for (let i = 0; i < setCount; i++) {
			const first = byIndex.get(i);
			if (first === undefined || !setOffsets[i]) {
				continue;
			}
			...
			for (const ro of offsets) {
				...
				for (let k = rr.u16(); k > 0; k--) {
					backtrack.push(rr.u16());
				}
				const input: number[] = [];
				for (let k = rr.u16() - 1; k > 0; k--) {
					input.push(rr.u16());
				}
				const lookahead: number[] = [];
				for (let k = rr.u16(); k > 0; k--) {
					lookahead.push(rr.u16());
				}
		...
		return {
			...
			apply(glyphs, i) {
				...
				for (const rule of rules) {
					...
					if (matched) {
						return runNested(glyphs, i, inputCount, rule.records);
					}
				}
				return null;
			},
	...
	if (format === 3) {
		...
		for (let k = r.u16(); k > 0; k--) {
			backtrack.push(coverage(buf, off + r.u16()));
		}
		const input: CoverageMap[] = [];
		for (let k = r.u16(); k > 0; k--) {
			input.push(coverage(buf, off + r.u16()));
		}
		const lookahead: CoverageMap[] = [];
		for (let k = r.u16(); k > 0; k--) {
			lookahead.push(coverage(buf, off + r.u16()));
		}
		const records: Array<{ sequenceIndex: number; lookupListIndex: number }> = [];
		for (let k = r.u16(); k > 0; k--) {
		...
		return {
			...
			apply(glyphs, i) {
				...
				for (let k = 0; k < input.length; k++) {
					if (!(input[k] as CoverageMap).has(glyphs[i + k] as number)) {
						return null;
					}
				}
...
function buildGsub(buf: Buffer): Substitutions {
	...
	function getLookup(index: number): Substitution | null {
		...
		for (const sub of lookup.subs) {
			...
			if (type === 1) {
				subtables.push(singleSubst(buf, off));
			} else if (type === 4) {
				subtables.push(ligatureSubst(buf, off));
			} else if (type === 6) {
				subtables.push(chainContextSubst(buf, off, getLookup));
			}
		}
