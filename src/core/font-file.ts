...
export class Reader {
  ...
  u8(): number { return this.b[this.p++] as number; }
  ...
  tag(): string { const v = this.b.toString('ascii', this.p, this.p + 4); this.p += 4; return v; }
...
export function readWoff2(path: string): Record<string, FontTable> {
  ...
  r.u32(); r.u32(); r.u32();        // metaOffset, metaLength, metaOrigLength
  r.u32(); r.u32();                 // privOffset, privLength
  ...
  for (let i = 0; i < numTables; i++) {
    ...
    const tag = tagIndex === 63 ? buf.toString('ascii', r.p, (r.p += 4)) : (KNOWN_TAGS[tagIndex] as string);
...
function parseCmap(buf: Buffer): Map<number, number> {
  ...
  if (format === 12) {
    sub.u16(); sub.u32(); sub.u32(); // reserved, length, language
