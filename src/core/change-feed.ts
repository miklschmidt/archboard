...
class ChangeFeed extends EventEmitter {
   ...
   */
  record(key: string, identity: BoardIdentity, read: () => ServerElement[], origin: ChangeOrigin): void {
  ...
  settle(key: string): ChangeEvent | null {
    ...
    logger.info(`Change event ${event.cursor} on "${key}" (${event.origin}, ${event.significance}): ${event.headline}`);
   ...
   */
  coalesce(since: number, board: string): { since: string; change: SemanticChange; events: ChangeEvent[]; cursor: number } | null {
    const watch = this.watches.get(board);
    if (!watch) return null;
    const events = this.since(since, board);
    if (events.length === 0) {
      return { since: watch.baselineAt, change: diffBoardStates(watch.baseline, watch.baseline, watch.identity, board), events, cursor: this.cursor };
    }
