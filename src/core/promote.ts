 ...
 *
 * There is no default, and that is the whole decision (ADR 0010). A surface
...
export function resolveBinding(request: BindingRequest, origin: BindingOrigin): ResolvedBinding {
  ...
  if (path.isAbsolute(raw)) {
    ...
    throw new PromotionError(
      ...
      'process happened to be started in, which nobody chose (ADR 0010). Name what you mean instead: an ' +
