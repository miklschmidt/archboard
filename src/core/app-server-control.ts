...
// thread that something happened. MCP cannot do it — Codex discards every
// server-initiated notification into tracing logs and never subscribes to
// resources (ADR 0005) — so push means this socket or nothing.
//
...
export function checkSocket(socketPath = controlSocketPath()): SocketCheck {
  try {
    ...
    return {
      ...
      ...(ownedByUs ? {} : { problem: `the socket belongs to uid ${stat.uid}, not to this process` })
    };
  } catch {
    return {
      path: socketPath,
      ...
      isSocket: false,
      ownedByUs: false,
      problem: 'no app-server control socket — the Codex app-server daemon is not running (or CODEX_HOME points elsewhere)'
    };
  }
}
...
export class AppServerControl extends EventEmitter {
  ...
  connect(): Promise<void> {
    ...
    this.ready = new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        fail(new Error(`the app-server did not answer initialize within ${this.requestTimeoutMs}ms`));
        ws.terminate();
      ...
      ws.on('open', () => {
        ws.send(JSON.stringify({
          ...
          method: 'initialize',
          params: {
            clientInfo: { name: this.clientName, title: 'archboard canvas', version: this.clientVersion },
      ...
      ws.on('message', raw => {
        ...
        if (message.id === 'initialize') {
          clearTimeout(timer);
          if (message.error) {
            fail(new Error(`initialize was refused: ${message.error.message ?? JSON.stringify(message.error)}`));
            return;
          }
  ...
  private handle(message: any): void {
    ...
    if (message.method && message.id !== undefined) {
      ...
      logger.info(`app-server asked archboard to handle "${message.method}"; declining — archboard is a listener on this socket`);
      this.ws?.send(JSON.stringify({
        id: message.id,
        error: { code: -32601, message: 'archboard does not implement app-server client requests' }
      }));
      return;
    }

    if (message.method) {
      this.emit('notification', { method: message.method, params: message.params } as ControlNotification);
    }
  }
  ...
  async call<T = any>(method: string, params: Record<string, unknown> = {}): Promise<T> {
    await this.connect();
    const ws = this.ws;
    if (!ws || ws.readyState !== WebSocket.OPEN) throw new Error('the app-server connection is not open');
    const id = this.nextId++;
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id);
        reject(new Error(`the app-server did not answer ${method} within ${this.requestTimeoutMs}ms`));
      }, this.requestTimeoutMs);
      timer.unref?.();
  ...
  // ---- the three verbs archboard uses -------------------------------------

  /** Thread ids the daemon currently holds in memory. Sorted by id, not recency. */
  listLoadedThreads(): Promise<{ data: string[]; nextCursor: string | null }> {
