...
function header(buf: Buffer, scripts: readonly string[]): Header {
  ...
  for (let i = 0; i < scriptCount; i++) {
    ...
    lr.u16(); lr.u16();             // lookupOrder, requiredFeatureIndex
    const n = lr.u16();
...
function deExtend(buf: Buffer, type: number, off: number, extensionType: number): { type: number; off: number } {
    ...
    kern(first: number, second: number): number {
      ...
      for (const subtables of lookups) {
        for (const subtable of subtables) {
          ...
          if (value !== undefined) { total += value; break; }
        }
      }
      return total;
    }
...
function ligatureSubst(buf: Buffer, off: number): Substitution {
  ...
  for (let i = 0; i < setCount; i++) {
    ...
    list.sort((a, b) => b.components.length - a.components.length);
    ...
    apply(glyphs, i) {
      ...
      for (const ligature of list) {
        ...
        for (let k = 0; k < ligature.components.length; k++) {
          if (glyphs[i + 1 + k] !== ligature.components[k]) { matched = false; break; }
        }
...
function chainContextSubst(
  ...
  if (format === 1) {
    ...
    for (let i = 0; i < setCount; i++) {
      ...
      rules.sort((a, b) => b.input.length - a.input.length);
      ...
      apply(glyphs, i) {
        ...
        for (const rule of rules) {
          ...
          for (let k = 0; k < rule.backtrack.length; k++) {
            if (glyphs[i - 1 - k] !== rule.backtrack[k]) { matched = false; break; }
          }
          if (matched) {
            for (let k = 0; k < rule.input.length; k++) {
              if (glyphs[i + 1 + k] !== rule.input[k]) { matched = false; break; }
            }
          }
          const inputCount = rule.input.length + 1;
          if (matched) {
            for (let k = 0; k < rule.lookahead.length; k++) {
              if (glyphs[i + inputCount + k] !== rule.lookahead[k]) { matched = false; break; }
            }
          }
    ...
    return {
      apply(glyphs, i) {
        ...
        for (let k = 0; k < lookahead.length; k++) {
          if (!(lookahead[k] as CoverageMap).has(glyphs[i + input.length + k] as number)) return null;
        }
...
export function buildGsub(buf: Buffer): Substitutions {
  ...
  function getLookup(index: number): Substitution | null {
    ...
    const built1: Substitution | null = subtables.length > 0
      ? {
        apply(glyphs, i) {
          for (const subtable of subtables) {
    ...
    substitute(glyphs: readonly number[]): number[] {
      ...
      for (const lookup of lookups) {
        ...
        while (i < out.length) {
          ...
          if (hit) { next.push(...hit.glyphs); i += hit.consumed; }
          else { next.push(out[i] as number); i++; }
        }
        out = next;
      }
      return out;
